...
- **Ask Cmdr tool-line labels are a `正在…` / `已…` pair, with `查看` for reading contents.** Every
  `askCmdr.tool.*.doing` opens with `正在` and its `.done` twin with `已`, on the same verb phrase
  (`正在查看文件内容` / `已查看文件内容`). Reading what's inside something is `查看…内容` (or `读取` for the photo
  facts tool); keep `显示` for the View menu. Per-term evidence: `glossary.md` § Ask Cmdr inspect-file consent.
