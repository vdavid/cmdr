...
Sources for `lo/`: Microsoft terminology (`LAO.tbx`) and a near-complete Xfce Thunar catalog (`lo/xfce-thunar/thunar.po`,
~1,030 of ~1,074 strings, `nplurals=1`). No macOS folder, Apple ships no Lao Finder strings (see Decision points).
...
branching is needed (confirmed by Thunar's `nplurals=1; plural=0`). The `desktop-i18n-plural` check requires every plural
message to cover the categories this language needs (here, just `other`).
...
add to it as you settle terms, each sourced from the reference pile (`_ignored/i18n/lo/`; recipes in
