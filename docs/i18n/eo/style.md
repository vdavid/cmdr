...
This is the language base (`eo`). Esperanto is a constructed auxiliary language with no regional or national variant
(by design), so there is exactly one `eo`.
...
- **Length: roughly comparable to English**, sometimes slightly longer due to agglutinative compounds and the
  consistent grammatical endings. Low overflow risk relative to German/Welsh. Confidence: high.
...
add to it as you settle terms, each sourced from the reference pile (`_ignored/i18n/eo/`; recipes in
