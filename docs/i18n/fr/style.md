...
- **Punctuation: a space before `: ; ! ?`, and guillemets « ».** French typography wants a thin space here; this catalog
  ...
  the exact characters, and the ICU apostrophe trap. Flagged here because the space-before-`:` is the French convention
  most likely to be dropped by a translator working from English copy. Confidence: confirmed (French typography; macOS
  follows it).
...
- **« Voici ce que vous pouvez essayer : »** ouvre chaque liste de suggestions à puces (« Here's what to try: »), avec
  l'espace ASCII avant le deux-points. Les jetons de volets (`{system_settings}`, `{privacy_and_security}`,
...
- **Le tas de références n'est pas sur toutes les machines.** `_ignored/i18n/fr/` est ignoré par git et n'existe que sur
  ...
  d'abord le chemin absolu du clone principal). Repli documenté : miner directement les paquets macOS installés
  (`.loctable` via `plutil -convert json`, recette dans `docs/i18n/reference-pile/how-to-mine.md`), en datant chaque
  citation par la version d'OS. Pour un terme qu'Apple n'a pas, la source de premier rang est l'éditeur du produit
  lui-même (ici la documentation Android en français).
