...
- **`rollbackTooltipStopAndMoveBack` → `停止，並把目前為止移動過的每一個檔案放回原處`** · 句式沿用同胞鍵 `rollbackTooltip`（`停止，並…`），`放回原處` 是目錄裡已定的說法（`cancelRollback.doneMovingBack`「放回原處」）· `high`。❌ 不用 `刪除`：復原一次移動不會刪掉任何東西。
- **`rollbackAlreadyLandedTooltip`** · 前半句沿用 `cancelRollback.moveAlreadyLanded` 的說法（「已經在目標位置了」），`復原` 是已定的術語（`rollbackUnavailableTooltip`），`取消` 直接用旁邊按鈕自己的標籤（`fileOperations.button.cancel`），依目錄慣例加上引號 · `high`。
