...
Working notes for translating Cmdr into Sinhala (Sri Lanka). Read `../README.md` for how this fits the translation
process, and the app-wide `docs/style-guide.md` for the English voice these notes carry into Sinhala.
...
- **Numbers and dates come from the formatter layer.** `formatNumber()`/`formatByteSize()` produce locale-correct
  output; never hardcode separators. Sinhala uses Western (Arabic) digits in modern UI.
...
sourced from the reference pile (`_ignored/i18n/si/`; recipes in `docs/i18n/reference-pile/how-to-mine.md`); never guess
