...
Open questions for a future native Brazilian Portuguese reviewer. Not translator input: every item below already ships
a reasoned value, recorded in `terms.json` or `decisions.md`. Remove an item once a reviewer settles it, and move the
...
- **`online` unhyphenated** (`commands.cloudRemoveDownload.description`, `fileOperations.delete.cloudOnlineOnly*`):
  the catalog writes `online`; the VOLP spelling is `on-line`.
- **Apple app names localized against the `@key`** (`settings.advanced.showSafeSaveFiles.description`: `o Editor de
  Texto e a Pré-Visualização`): the `@key` asks to keep `TextEdit` / `Preview`; macOS pt-BR names them this way.
- **Smaller tentative calls**: `comparador` (`queryUi.*.aria.comparator`), `manifesto`
  ...
  (`settings.askCmdr.spend.disclaimer`), `alternador de apps` (`shortcuts.system.appSwitcher`), `Consulta` for the
  query criterion (`queryUi.results.criteria.query`).
...
- **`Fixar no seletor`** (`menu.network.pinToSwitcher`): the full published term is `seletor de volumes`; the short
  form follows the English `switcher`. Confirm, or accept a five-word `Fixar no seletor de volumes`.
...
- **The online-only delete warnings** (`fileOperations.delete.cloudOnlineOnlyMixedWarning`, `.cloudOnlineOnlyAllWarning`,
  `.cloudOnlineOnlyHandedBack`): drafted without human review; also check overflow, since the warning is long and sits
  in a narrow band above the file list.
