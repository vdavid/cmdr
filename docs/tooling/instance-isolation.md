...
| `tauri-plugin-store` (`settings.json`) | When `CMDR_DATA_DIR` is set, the frontend loads the store from `<resolved_data_dir>/settings.json` (absolute path) via `get_isolated_settings_path`, matching the Rust loader. Production (no `CMDR_DATA_DIR`) keeps the plain `BaseDirectory::AppData` path, byte-identical. Don't rely on the generated config's identifier alone: the Playwright E2E checker launches the pre-built binary directly (no wrapper, no `-c`), and a runtime `-c` identifier override does NOT redirect `app_data_dir()`, so without this the store reads the real prod `settings.json`. | [`settings.rs`](../../apps/desktop/src-tauri/src/commands/settings.rs), [`settings-store.ts`](../../apps/desktop/src/lib/settings/settings-store.ts) |
