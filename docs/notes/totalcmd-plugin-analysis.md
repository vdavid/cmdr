...
| 72  | MakeBAT (write-only — generate .bat)                                                                                            | B   | Paradigm: write-only "packer" as a generator. Cool but Cmdr likely doesn't need it.                                                                                  |
| 73  | Audioconverter / Mod2Wav                                                                                                        | C   | Format-conversion via packer. Real workflow but better as a transform pipeline.                                                                                      |
| 74  | Graphic Converter / TotalRSZ / BMC                                                                                              | C   | Same — image conversion/resize via packer UX. Cmdr could surface this as a transform action.                                                                         |
...
| 2   | **Universal media** (MediaInfo, MediaInfoWDX, TCMediaInfo, Media, MKInfoCP, MediaTime)                                                                                                                                                               | A   | `mediainfo` is the canonical lib for video/audio/container metadata. Cmdr should integrate it. `MediaTime` adds _recursive duration sum_ — useful aggregation pattern.                                                                                                                                                       |
...
- **Content plugins are the most AI-aligned of the four plugin types.** A + B = ~59% — by far the highest signal/noise
  of any plugin category we've reviewed. **All of category B is "structured data as a column"** — and that's exactly
  what an AI-native file manager produces by default. Cmdr can deliver most of B without writing per-format plugins, by
  having the AI describe any file into well-typed columns.
...
| 27  | **X.509 / certs / signatures** (CertView, ASNView, CertificateInfo, SignatureInfo)                                                                                                                                                                                                                                                                               | A   | Cmdr should ship a cert previewer for `.cer/.pem/.p7b/.p12/.crl/.csr` — uses real Rust crates (`x509-parser`, `rustls-pemfile`).                                                                                |
...
1. **Process isolation & crash containment.** TC plugins are in-process DLLs — one bad plugin tanks the app. What's
   ...
   (fast, dangerous)? Pick before you design types — it dictates the ABI.
...
9. **Stability/versioning commitment.** TC's API has been stable for decades — that's a feature plugin authors love.
   Will Cmdr commit? SemVer + capability negotiation is the modern answer.
...
Before drawing types: **build a "skateboard" — one full vertical slice of the simplest plugin type (Column), end-to-end,
in one language (Rust or TS)**, with: manifest, scopes, sandboxed subprocess, JSON-RPC contract, hot reload,
install/consent UI, audit log entry. ~1–2 weeks of work.
