...
- **The defect is real and the fix lands.** 1.055 s → 588 ms at window 4 is **1.79×**, spreads
  ([945.6-1861] vs [516.5-1108] ms) disjoint at the medians and overlapping only in the tails. Before this change every
  row in that table would have read like row 1, because the width had nowhere to apply.
...
- **MTP was not measured and cannot be.** Its `max_concurrent_ops()` of 1 makes the window serial by construction, which
  is asserted rather than timed (`merge_window_tests.rs::a_cap_of_one_keeps_a_subtree_strictly_one_operation_at_a_time`).
