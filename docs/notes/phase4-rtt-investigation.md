...
Landed once smb2 0.7.1 shipped `Tree::download(&self, conn: &mut Connection, path)`, which lets cmdr clone a single
...
- **Local-side write latency.** A 10 KB `std::fs::write` via `LocalPosixVolume::write_from_stream` on macOS takes a few
  hundred µs, not 30 ms, so shouldn't matter.
...
on the wire, and the receiver side multiplexes them on the same SMB session).
