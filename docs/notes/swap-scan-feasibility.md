...
that voids its headline promise (the `scan_completed_at` clear), and one performance own-goal (search reload thrash). A
**separate DB file** rather than shadow tables in the same file avoids three of the six problem areas outright; see "The
file-swap variant" at the end.
...
**The problem the plan doesn't assume**: `MutationTracker::bump` has no notion of _which_ table was written. A shadow
...
**Not determined by reading**: what happens to a load that is _mid-iteration_ when the swap commits on the writer
...
connection run `DROP TABLE entries; ALTER TABLE entries_new RENAME TO entries;` mid-iteration, and asserts the iteration
completes with the pre-swap row count and no error.
...
- **The per-navigation verifier** (`indexing/verifier.rs`), which reads through `ReadPool` and is gated off while a scan
  runs (`verifier.rs:63-66`).
...
**The big de-risker**: `start_scan` starts the `DriveWatcher` and _buffers_ FSEvents; the live loop only starts in the
...
writers. I see no BUSY hazard from the DDL itself. What I could _not_ determine by reading is how long the `DROP TABLE`
of a ~1.1 GB table takes with `auto_vacuum = INCREMENTAL` on (it must update pointer-map pages for every freed page) and
how much WAL it generates. Needs measurement.
...
`current_epoch` is bumped at scan start (`manager.rs`, Step 0a'), the calibration keys describe the last _completed_
...
- DDL is transactional in SQLite, so `DROP TABLE entries; ALTER TABLE entries_new RENAME TO entries;` in one transaction
  is atomic and crash-safe under WAL. Other connections never observe the intermediate state.
...
tables _and_ a populated `entries_new` behind. Not corruption, but the recovery path must be idempotent, not "assume the
swap either happened or never started".
...
- Add WAL. `journal_size_limit` is 64 MiB (`store/mod.rs:547`) but that only trims the file _after_ a checkpoint, and
...
- **`auto_vacuum = INCREMENTAL` does not reclaim anything on its own.** It only makes reclaim possible via
  ...
  (`indexing/writer/maintenance.rs:143-165`: skip below 1,000 free pages, 2,000/tick up to 20,000, 20,000/tick above). A
  ~274,000-page freelist at 20,000 pages per 30 s tick is ~14 ticks ≈ **7 minutes** before the file is back to its
...
1. **`scan_completed_at` is cleared at scan start** (`indexing/manager.rs`, Step 0a, the
   `DeleteMeta("scan_completed_at")` send). If a swap scan is interrupted, `entries` is fully intact but the completion
   marker is gone, so the next scan evaluates `local_rescan_reconciles(entry_count, prior_scan_completed = false)`
...
2. **Orphan `entries_new` / `dir_stats_new` survive forever.** Nothing enumerates `sqlite_master`; `create_tables` is
   ...
   leftover shadow table costs ~1.1 GB invisibly, and the next scan's `CREATE TABLE entries_new` finds a populated table
   and would append to garbage. Needs an explicit `DROP TABLE IF EXISTS entries_new, dir_stats_new` at open, plus after
   every swap and every scan abort.
...
- Cleanup on interruption is `remove_file`, and detecting a leftover is a `path.exists()` instead of a `sqlite_master`
  query.
- Existing readers keep the old inode alive until they close it (POSIX unlink semantics), which is exactly the "index
  stays readable throughout" property; `ReadPool::invalidate()` (`indexing/enrichment.rs:43`) already exists to flip
  them onto the new file, and `state.rs` already calls it on three lifecycle paths.
...
5. Does the bulk parallel walker actually keep its 68 s throughput when the DB file already holds 1.12 GB of live data
   (page-cache pressure, more file growth, checkpoint interference)? The 68 s figure was measured against an empty file.
