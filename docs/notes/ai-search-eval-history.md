...
Each round lists pipeline, query count, reliability, and the key changes made after it.

- **R1** (v1, 6 queries, reliability n/a): glob limitation docs, naming conventions, category-to-extension mapping, size
  inference, macOS screenshots, default code exclusions.
- **R2** (v1, 19 queries, 72%, 8/29 empty, 15s timeout): HTTP timeout 15→30s, pass-1 JSON in refinement context,
  "preserve flags" rule, regression guard.
- **R3** (v1, 29 queries, 90%, 26/29): prompt compacted 8234→3200 chars, extracted to constants.
- **R4** (v1, 29 queries, 90%, 3 empty): restored "be broad" framing, prominent regex constraint, three examples back as
  behavioral anchors, "ALWAYS include namePattern" rule, empty-pattern guard in `search()`.
- **R5** (v1, 30 queries, 93%, 28/30): final v1 round. Fixed remaining R4 regressions, established the 30-query catalog,
  and identified architectural limits of the JSON generation approach.
- **R6-cloud** (v2, 30 queries, 100%, 30/30): v2 pipeline (classification prompt + Rust builder). Zero parse failures,
  zero MCP timeouts.
- **R6-local** (v2, 30 queries, 100%, 30/30): same pipeline on the 2B local model. All 30 queries return valid
  classification output.
- **R7-local** (v2, 30 queries, 100%, 30/30): post-fix eval on the 2B model. Great/Perfect 43%, Bad down from 43%→20%.
  Fixes: shell-scripts type, last_6_months, redundant keyword detection, .key removal, prompt improvements.
...
| 3   | node_modules space               | Bad           | Good          | Good          | Okay (768)    | `*node_modules*` glob, is_dir=true, min_size=100MB                            | R7: regressed. Keywords `(node_modules\|folders\|taking)`, lost `folders: yes` + `size: large`  |
...
| --- | -------------------------------- | -------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| 1   | `large files modified this week` | `time: this_week` + `size: large`                                                                  | Yes. Exercises both `time` and `size` enums; Rust derives the regex and date range from there. |
