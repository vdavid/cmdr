...
| `file-explorer/git/`        | Breadcrumb chip, status-column helpers, reactive `RepoInfo` store. M2 adds Lucide-rendered git portal icons via `selection/FileIcon.svelte` |
...
| `file_system/git/`              | Git browser. M1: repo discovery, repo info, status, watcher, friendly errors. M2: virtual `.git` portal (`branches/`, `tags/`, `raw/`) + `VolumeReadStream` for git blobs so cross-volume copy plucks files from refs. M3: `commits/`, `stash/`, `worktrees/`, `submodules/` — same tree-walking pipeline for branch/tag/commit/stash; `redirect_to_path` pivots worktree/submodule entries to their working dir |
