...
| `search/`                   | Whole-drive file search dialog: orchestrator + unified search bar with mode chips, filter chips with popovers, AI transparency strip, empty state, recent-searches footer + popover, results table with path pills + row menus, "Open in pane" handoff (snapshot store + virtual `search-results` volume), MCP `open_search_dialog` tool |
