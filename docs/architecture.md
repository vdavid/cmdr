...
| `file_system/git/`              | Git browser. M1: repo discovery, repo info, status, watcher, friendly errors. M2: virtual `.git` portal (`branches/`, `tags/`, `raw/`) + `VolumeReadStream` for git blobs so cross-volume copy plucks files from refs |
