...
- [x] Verify signing:
    ...
    You should see your signing identity and `Authority=Developer ID Application: ...` in the output. If it says
    `ad-hoc` or has no identity, the certificate isn't in your keychain or the `signingIdentity` in `tauri.conf.json`
    doesn't match.
