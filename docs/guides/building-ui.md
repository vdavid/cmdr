...
the house primitive in [`apps/desktop/src/lib/ui/`](../../apps/desktop/src/lib/ui/) instead of hand-rolling chrome or a
raw control. Two things enforce it: the `cmdr/prefer-ui-primitive` ESLint rule (no raw native controls) and the
`ui-primitive-coverage` check (every primitive appears in the live catalog).

The canonical token and pattern catalog is [`docs/design-system.md`](../design-system.md). The LIVE catalog, every
primitive rendered flat with all variants and states, is Debug window (`⌘D`) → "Components"
(`apps/desktop/src/routes/dev/components/`). Open it before building; it's faster than reading source.

## Building a dialog

Use [`ModalDialog.svelte`](../../apps/desktop/src/lib/ui/ModalDialog.svelte). It owns the overlay scrim, focus trap,
Escape-to-close, the MCP dialog registry wiring, and the standard body padding. Don't hand-roll dialog chrome, don't set
your own body padding, and don't add a focus trap yourself: the primitive already does all of it.
...
- Optional props: `resizable` (user-draggable resize) and `padded` (toggle the standard body padding for full-bleed
  content). Both are documented in [`design-system.md`](../design-system.md) § Dialogs.
- Details and gotchas: [`lib/ui/CLAUDE.md`](../../apps/desktop/src/lib/ui/CLAUDE.md) and its `DETAILS.md`.

For a full-screen commit flow (onboarding, consent, multi-step setup the user can't cancel), use a soft sheet:
[`OnboardingWizard.svelte`](../../apps/desktop/src/lib/onboarding/OnboardingWizard.svelte) and the `--sheet-*` tokens.
The sheet-vs-dialog decision table is in [`design-system.md`](../design-system.md) § Soft sheets.
...
Compose from the settings components in
[`apps/desktop/src/lib/settings/components/`](../../apps/desktop/src/lib/settings/components/):
...
entry plus row) is its own procedure: [`adding-a-new-setting.md`](adding-a-new-setting.md).

## Building a window

A new top-level window (like Settings or the File viewer) is a route, an opener, a capability file, and shell wiring.
Follow [`adding-a-window.md`](adding-a-window.md); missing capabilities fail silently, so read the capabilities section.
...
4. An entry in [`design-system.md`](../design-system.md) § Component patterns.

The canonical statement of this contract lives in [`lib/ui/CLAUDE.md`](../../apps/desktop/src/lib/ui/CLAUDE.md); read it
before adding or changing a primitive. Prefer a primitive over a raw native control everywhere
(`cmdr/prefer-ui-primitive`).
