...
- Optional props: `resizable` (drag any edge or corner; `"horizontal"` when nothing inside uses extra height), `fillBody`
  (fixed-height frame), and the rest, documented in `../design-system.md` § Dialogs. A dialog that renders a path or a
  filename takes `resizable`, and gives every shortened string a `use:tooltip` carrying the full text. The body's side
  inset is NOT one of them: `ModalDialog` always owns it, in any token (`dialog-inset.spec.ts` measures every gallery
  dialog and fails on a section that re-pays it).
