...
`IoError` variants with a `raw_os_error` are matched against 37+ macOS errno codes in
`friendly_error_from_errno()`. Examples:
...
1. Add the match arm in `friendly_error_from_volume_error` (for `VolumeError` variants) or
   `friendly_error_from_errno` (for new errno codes)
