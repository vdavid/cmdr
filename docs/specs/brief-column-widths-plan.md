...
settles can we ask the backend for new widths. The backend always uses the _currently cached_ font ID — the FE doesn't
need to pass it in.
...
- `apps/desktop/src/lib/file-explorer/views/BriefList.svelte`:
  ...
  - Add `MAX_BRIEF_COLUMN_WIDTH = 300` (new constant). The current code at `BriefList.svelte:139` uses `200` as a
    fallback-only floor (`Math.min(200, ...)`) when `backendMaxWidth` is undefined — that's not a "cap" in the new
  ...
  - `handleKeyNavigation` (line 451) — replace `const visibleColumns = Math.ceil(containerWidth / maxFilenameWidth)`
    ...
    give the range in O(log n). Don't derive from `virtualWindow.endIndex - startIndex - 2 * bufferColumns` — that
...
itemsPerColumn possibly unchanged, capPx possibly unchanged) may fire — the `prevItemsPerColumn` / `prevCapPx` guards
