...
- The current error display is a single `<div class="error-message">{error}</div>` showing the raw Rust `Display`
  output. It's functional but scary, ugly, and unhelpful. It violates every principle in our style guide ("never use the
...
panics are rare internal errors, not filesystem errors, so no `FriendlyError` enrichment is appropriate).
