...
- `lib.rs`:
  ...
  - Replace with a minimal init script: just `window.__PW_ACTIVE__ = true` as a readiness flag for the test fixture (the
    fixture's `waitForFunction('window.__PW_ACTIVE__')` checks this before running tests).
...
- `commands.rs`:
  - Add a Tauri command for receiving results from the webview:
    ...
    ```
    This looks up `id` in the pending map and sends the result through the oneshot channel. **Important**: Don't apply
    `#[serde(rename_all = "camelCase")]` — the field names must match the JS object keys exactly (`id`, `ok`, `data`,
    `error`).
