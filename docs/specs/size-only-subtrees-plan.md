...
**Status**: SPECCED, not started. **Owner**: David. **Date**: 2026-08-04. **Baseline**: `main` at `5cfa4ea50`.
...
Two things fall out of that table and they shape the design:
...
it is why the feature is sound rather than approximate: a per-directory total is a lossless summary of the per-file
rows for everything the aggregate needs.
...
- **Disk-space attribution, including drill-down.** Directory rows and their `dir_stats` survive, so `target/ -> debug/
  -> build/` still shows real recursive sizes. The use case David named as non-negotiable.
- **Live, exact size tracking**, per the mechanism above.
- **Directory names in search.** Only file rows go away.
...
- **Media enrichment finds no images there**, since it walks file rows. Correct for build output, and named here
  because it is a behavior change rather than an oversight.
...
A `getattrlistbulk` sweep of ~95k entries is tens of milliseconds against ~190 ms of row writes, so coalescing wins
most on the pathological case. It also matches what the feature is for: disk-space attribution tolerates a size that is
...
byte count live in completely different places, which is the clearest possible argument for storing folder totals
rather than file rows.
...
directory tree. The summing path MUST carry the existing rule
(`db.logical_size.is_none() && snap.nlink > 1` => mtime only), and M1 should assert it against a fixture with real
hardlinks.

## The model

One flag on the subtree ROOT's `entries` row, inherited downward: this directory and everything beneath it is
size-only.
...
- **The reconciler must not read a size-only directory's missing file rows as deletions.** The likeliest silent bug:
  `diff_dir_against_db` sees files live and none stored. It must know the mode BEFORE diffing. Same for the verifier
  and `count_children_capped`.
- **Ancestor aggregates stay exact.** The bytes must still roll up to `~` and `/`, or the feature breaks the use case
  it exists for.
...
- **The flag is inherited**, so a nested `target/` inside a size-only tree is not a second decision.

## Milestones

**M1 - direct totals, everywhere.** Add the three `dir_stats` columns, populate them on every write path, bump
`SCHEMA_VERSION`. No behavior change yet: the normal path keeps its file rows, and the new numbers are asserted to
equal what a child scan computes. **This is the load-bearing milestone** and it is independently useful, because it
makes a directory's own bytes an O(1) read.

**M2 - the flag and the read path.** Schema column, inheritance, and every reader that must respect it (listing,
search, media walk, verifier, reconciler diff). Mark subtrees only from a test hook; no policy yet.

**M3 - the write path, with coalescing.** Scanner and reconciler skip file rows in a size-only subtree, computing direct totals and
propagating deltas from them. TDD, and the differential check is exact: the aggregate for a size-only subtree must
equal the aggregate the normal path produces for the same directory, including after a create, a modify, a delete, and
a `cargo clean`.
...
**M5 - measure it.** Re-run the table above against a rebuilt index: rows under `target/`, rows written per rebuild,
and CPU per hour under active cargo churn (`scratchpad/cmdr-churn-metrics.csv` holds the before-baseline). Don't call
this done on a row count; the claim is about CPU under churn.
...
- **A schema bump forces a full rescan** (~10 min on a big NAS). Acceptable per "rebuild, don't migrate", but worth
  riding along with other schema work rather than being its own forced rescan.
...
- **Interaction with `unindexed-search-plan.md`.** That plan converges a volume toward full coverage by walking gaps; a
  size-only subtree is a permanent, intentional gap. Resolve before M3 or search will re-walk these forever.

## Open questions for David

1. **Should size-only directories still appear in search results by name?** The spec assumes yes (dir rows survive).
2. **Nested marked roots**: inherited (the spec's assumption) or a distinct decision?
3. **The rescan after a schema bump** is the main user-visible cost. Pair it with other schema work, or ship alone?
