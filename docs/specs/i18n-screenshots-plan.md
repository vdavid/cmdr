...
- **Paid-license states** (About/commercial-reminder/expiration): the `CMDR_MOCK_LICENSE=commercial` dev flag —
  automatable.
...
`pnpm check -q` per milestone (bare, never piped — `no-tail-checker`). Specifically: `svelte-check`,
...
- **Snapshot-resolved strings need capture-on-before-trigger, not `rerender()`-after (extends Decision 1/2 + Decision
  ...
  strings resolve as SNAPSHOTS in plain `.ts` at emit time — e.g. `settings-applier.ts:239` `addToast(tString(...))`,
  and the dynamic
...
- **Some inventory surfaces are "stage-or-defer", not flatly drivable (refines the inventory):** crash-report dialog
  (the app must actually crash — can't screenshot a crashed webview), commercial-reminder/EXPIRATION (needs
  `CMDR_MOCK_LICENSE`
  - possibly an expired-state mock / time manipulation — verify the mock supports expired, not just `commercial`),
    auto-dismiss toasts (capture must land in the visible window — pin the toast or capture fast),
