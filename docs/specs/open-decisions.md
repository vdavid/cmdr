...
- **Problem**: a file that exhausts its retries ends the operation it belongs to. Copy 700 files, have file 200 fail
  ...
  could not do.
- **Impact**: the user re-runs the whole operation to rescue the tail, on top of an already slow transfer, and a flaky
  remote (a sleeping NAS, a dropped SMB session) turns one bad file into an abandoned job. No report has named this yet,
  so the frequency is unmeasured.
