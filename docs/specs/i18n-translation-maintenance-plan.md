...
PRESERVING every `{placeholder}`, `<tag>`, and ICU `plural`/`select` structure, and recording each key's source hash) is
a valid, loadable locale that MUST pass every check. It's simultaneously: the overflow-testing tool (drive the app in
...
`EXIT_CLEAN`/`EXIT_ISSUES`/`EXIT_ERROR` contract the Go wrapper maps; see that file's header for the pattern an M3 check
follows). A new M3 check is a thin Node script (en catalog + one locale catalog in → per-key findings out, via
...
- **Error-pipeline loud note** (not a check): `errors.*` go through `getMessage()` (raw, no ICU), so translators must
  NOT add ICU syntax there. Loud note in `messages/DETAILS.md` (errors section) + the translator guide.
...
- **The translator-agent context/system prompt** to hand an agent: per-string `@key` context + screenshot/note + the
  ...
  uncontrolled runtime inserts, so structure sentences to tolerate them; (b) fragment/concatenation keys assemble via a
  named `*Join`, so respect word order. Plus principle 6: human reviews translated copy.
...
- The runtime locale resolver + a language selector: land with the first real locale (this plan keeps the seam clean and
