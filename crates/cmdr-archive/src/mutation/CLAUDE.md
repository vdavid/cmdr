...
- **Only zip is WRITABLE.** The write chokepoint is the host's `ensure_zip_writable` chokepoint (non-zip → typed
  `ReadOnlyDevice`, untouched). Don't route a non-zip archive here.
