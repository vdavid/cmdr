...
//! from the `Volume` trait and from any registry, just like the
//! [read core](crate::read). The host's archive-edit driver wraps [`mutator`] with
//! the real event sink, pause gate, and cancel intent; nothing calls
//! `ArchiveVolume`'s own mutation methods (they stay `NotSupported`).
//!
