...
- **`listing_is_watched` reflects the live [content watch](src/watch/CLAUDE.md)** — `true` only while the local content
  watch is established (never for a remote parent). `supports_watching` stays `false` (a generic FSEvents dir-watcher
  can't watch an archive-inner path).
