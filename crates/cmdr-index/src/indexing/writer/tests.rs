...
fn spawned_non_feeding_writer_does_not_bump_global_generation() {
    ...
    let writer = IndexWriter::spawn_for(&db_path, crate::NoopEventSink::shared(), false, "root".to_string()).unwrap();
...
fn mark_dirs_listed_does_not_bump_global_generation() {
    ...
    let writer = IndexWriter::spawn_for(&db_path, crate::NoopEventSink::shared(), true, "root".to_string()).unwrap();
...
fn non_root_writer_drain_clears_its_own_tracker_not_root() {
    ...
    let (db_path, _dir) = setup_db();
    let writer =
        IndexWriter::spawn_for(&db_path, crate::NoopEventSink::shared(), false, volume_id.to_string()).unwrap();
...
fn a_fatal_storage_error_stops_the_writer_and_trips_the_signal() {
    ...
    let signal = Arc::new(IndexFailureSignal::new(crate::NoopEventSink::shared()));
