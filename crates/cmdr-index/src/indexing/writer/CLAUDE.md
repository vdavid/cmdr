...
- **A fatal storage error STOPS + FAILS the index, never retries** (one incident: 12,700 warnings in 8 min). Typed via
  `IndexFailureSignal`, never a message substring; `BUSY`/`LOCKED` stay retried. Failed lifecycle:
