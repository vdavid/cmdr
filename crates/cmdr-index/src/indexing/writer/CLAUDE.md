...
- **`Abandoned` is the one `unreadable_cause` Cmdr RETRIES** (`abandoned_retry.rs`: a persisted per-volume 1 h → 4 h →
  24 h window, armed by the mark). ❌ Never a flat retry, ❌ never clear `Denied`/`Declined`.
