...
// subtree).
//
// ⚠️ They wait on `settle`, ❌ never on the flush alone: the handler QUEUES the
// ancestor and the writer rolls it up at its caught-up point, one hook run after
// the flush replies (`pending_rollups.rs`).

pub(super) fn dir_row(id: i64, parent_id: i64, name: &str) -> EntryRow {
    EntryRow {
        id,
        parent_id,
        name: name.into(),
...
pub(super) fn file_row(id: i64, parent_id: i64, name: &str, size: u64) -> EntryRow {
    EntryRow {
        id,
        parent_id,
        name: name.into(),
...
pub(super) fn set_epoch(db_path: &std::path::Path, entry_id: i64, epoch: u64) {
