...
- **The COVER walk inverts that cancel rule, is SCOPED, and is ADD-ONLY**: it stamps what it read on EVERY exit (a
  ...
  needs neither the virgin-root nor the empty-root refusal.
...
- **Descent is the volume's call** (`Volume::index_walk`); by default never into a link. ❌ Never trust `is_directory`
  alone: ADB sets it on a link.
...
- **A backend may fan `list_directory_for_scan` across an internal pool** (SMB's extra TCP sessions, via
  `begin`/`end_scan_session`); the in-flight budget still caps it, so pacing survives.

The pump, the cover walk, pacing, NAS dirs, walk scope, and empty roots: `DETAILS.md`. Read it before any non-trivial
