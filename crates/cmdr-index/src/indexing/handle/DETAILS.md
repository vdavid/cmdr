...
- **`cover(volume_id, frontier, dimension, cancel)`** — the walk half: it takes the frontier a coverage answer named and
  ...
  than a method on the handle because the handle can't leave the thread reading its batches (§ below). ⚠️ **A walk
  started here OUTRANKS the background**, and that is the whole difference between this door and the phase machine's:
  somebody is waiting on it, so a background walk over ground it names is asked to hand that ground over rather than
  keeping it. It is never asked to yield in turn. The mechanism, and who may be asked, is
