...
impl ReadPool {
    /// Open a pool over an index database, without going through the registry.
    /// For a host that has a database path and wants to read it directly, which
    /// `search/` does for volumes the lifecycle layer isn't holding open.
    pub fn new(db_path: PathBuf) -> Result<Self, IndexStoreError> {
    ...
    pub fn with_conn<T>(&self, f: impl FnOnce(&Connection) -> T) -> Result<T, String> {
