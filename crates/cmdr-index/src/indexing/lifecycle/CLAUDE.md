...
- **cover.rs** (+ **cover/bootstrap.rs**, what has to exist before a walk can run; **cover/live.rs**, which frontier
  roots are being walked right now) — the search-driven walk over a coverage frontier (`Index::cover`).
...
- **A cover walk reuses the RUNNING writer; no index ⇒ it stands one up** (`Activation::WriterOnly`: DB, epoch, writer,
  read handles; ❌ no scan or watcher, and `../store/` owns its `EXCLUSION_POLICY_KEY` stamp). Shares and phones are
  refused (M3d). ⚠️ A volume mid-SCAN isn't walked, and two walks over overlapping frontiers don't both take the ground
  (`cover/live.rs` claims roots): one writer doesn't stop `INSERT OR IGNORE` dropping a collider and orphaning its
  subtree.
...
- **TWO switches, master wins, and both gate BACKGROUND work only.** `indexing.enabled` hard-gates
  ...
  switches AND `user_disabled`: a read someone asked for, starting no scan or watcher, so refusing only makes their
  search silently wrong. ❌ Don't "fix" that into a refusal.
