...
/// fully self-contained. The genuinely pure decision logic lives — and is
...
pub(crate) struct ScanProgressReporter {
    ...
    writer: IndexWriter,
    /// Where scan progress goes (and, indirectly, the partial-aggregation
    /// directory refresh the writer fires).
...
impl ScanProgressReporter {
    /// Create a reporter for a scan of `volume_id`. The writer handle and sink are
    ...
    fn tick(&mut self) {
        ...
        self.events.emit(IndexEvent::ScanProgress {
