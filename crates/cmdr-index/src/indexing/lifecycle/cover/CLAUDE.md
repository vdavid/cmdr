...
- **Every primitive REPORTS the rows it CREATED**, the repair included (`emit` + a `ScanSummary`, ❌ never
  `(None, Covered)`): a search reads the rest off an arena predating the walk, so a silent one answers short and calls
  itself complete. ⚠️ Created only; re-sending a held row doubles it.
