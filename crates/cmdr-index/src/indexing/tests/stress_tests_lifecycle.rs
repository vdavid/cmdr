...
fn lifecycle_transitions_under_load() {
    ...
    let writer2 = IndexWriter::spawn(&db_path, crate::NoopEventSink::shared()).expect("spawn second writer");
    ...
    for cycle in 0..4 {
        let w = IndexWriter::spawn(&db_path, crate::NoopEventSink::shared())
    ...
    let w = IndexWriter::spawn(&db_path, crate::NoopEventSink::shared()).expect("spawn for flush test");
    w.send(WriteMessage::TruncateData).unwrap();
    ...
    let w = IndexWriter::spawn(&db_path, crate::NoopEventSink::shared()).expect("spawn for double shutdown test");
    w.send(WriteMessage::TruncateData).unwrap();
...
fn lifecycle_clean_start_populate_shutdown() {
    ...
    let writer2 = IndexWriter::spawn(&db_path, crate::NoopEventSink::shared()).expect("spawn post-shutdown writer");
    writer2.flush_blocking().unwrap();
    writer2.shutdown();
}
...
fn early_shutdown_during_active_writes() {
    ...
    let writer2 = IndexWriter::spawn(&db_path, crate::NoopEventSink::shared()).expect("spawn after early shutdown");
    writer2.flush_blocking().unwrap();
    writer2.shutdown();
}
...
fn rapid_start_stop_start_produces_clean_state() {
    ...
    let writer1 = IndexWriter::spawn(&db_path, crate::NoopEventSink::shared()).expect("spawn writer 1");
    ...
    let writer2 = IndexWriter::spawn(&db_path, crate::NoopEventSink::shared()).expect("spawn writer 2");
...
fn shutdown_with_mixed_queued_work() {
    ...
    let writer2 =
        IndexWriter::spawn(&db_path, crate::NoopEventSink::shared()).expect("spawn after mixed-work shutdown");
    writer2.flush_blocking().unwrap();
    writer2.shutdown();
}
...
fn disconnect_storm_writer_drain_holds_across_volumes() {
    ...
    for round in 0..ROUNDS {
        ...
        for db_path in order {
            ...
            let writer = IndexWriter::spawn_for(db_path, crate::NoopEventSink::shared(), false, "root".to_string())
    ...
    for db_path in [&db_a, &db_b] {
        let writer =
            IndexWriter::spawn(db_path, crate::NoopEventSink::shared()).expect("spawn after storm: DB not wedged");
        writer.send(WriteMessage::TruncateData).unwrap();
        let tree = build_synthetic_tree(2, 3, 4, 512);
        let tree_len = tree.len();
