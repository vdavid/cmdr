...
/// that restores the previous value on drop; hold [`host::test_lock`] first,
/// because the slots are process-wide.
pub mod host {
    /// What [`crate::IndexBuilder::install_for_test`] hands back: holding it keeps
    /// the fake host installed, dropping it puts the previous one back.
    pub use crate::indexing::handle::TestInstallGuard;
    /// Serializes tests that install a host. Two installing at once would see
    /// each other's volumes and events.
    pub use crate::indexing::handle::test_lock;
...
/// Watching what the index reports without an app to report to, and the sample
/// set a host's mapping is proved complete against.
pub mod events {
    pub use crate::indexing::events::{RecordingSink, one_of_every_kind};
}
...
#[cfg(all(any(test, feature = "testing"), target_os = "macos"))]
...
/// Whether a volume has a live index instance (initializing or running). The raw
/// registry answer, for a host-side test asserting that its own code started or
/// stopped one. Production code asks [`crate::Index::volume_status`] instead,
/// which reports the phase rather than a bit.
pub use crate::indexing::lifecycle::state::is_active as is_index_active;
