...
- **Drive full recompute off the bus `ScanCompleted`, the startup sweep, and the hourly `FULL_REFRESH_INTERVAL` tick, ❌
  ...
  gap is never wired.
- **Coalesce per volume through `PassCoordinator`**: one pass plus one re-run. Incremental has its OWN key, so a
  rescore and a full pass never block each other.
- **The full walk is O(dirs) in a SMALL CONSTANT, every part of that shape measured.** Dirs live in the shared `DirTree`;
  a folder is one `Copy` `IndexFolder`. ❌ No `EntryRow`, no `IndexStore::all_directories`, no stored path, no
  per-folder extension set, and ❌ never drop `for_each_file_child_by_parent`'s `ORDER BY parent_id`.
...
- **An incremental reads only the CHANGED SUBTREES** (`scoped_walk.rs`). ❌ Don't prune floored subtrees from the descent
  (a marker inside a `node_modules` still raises the folders above it), ❌ don't add a `folders.is_empty()` early return
  (an all-deleted batch is exactly the one whose rows must be CLEARED), ❌ don't raise `SCOPED_WALK_MAX_ORIGINS` /
...
- **A pass writes only what MOVED, keyed on the SIGNALS blob and ❌ never the score** (`../writer.rs`): a score drifts
  with `now_secs` on its own. The log carries BOTH `written` and `considered` — ❌ don't drop the latter, it's what makes
  a too-wide batch visible.
