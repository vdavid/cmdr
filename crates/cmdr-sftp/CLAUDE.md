...
- `volume/`: `mod.rs` (the volume, its params, `connect_sftp_volume`), `paths`, `query`, `mapping`, `volume_impl`,
  `testing` (the Docker fixtures).
...
- **❗ Every `#[ignore]`d test here is a Docker cell**, by construction: the lane runs `--run-ignored only` over the
  whole package. A measurement that must not gate CI needs an env gate instead.
