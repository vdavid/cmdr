...
describe('resolveSearchTargetVolume', () => {
  it('targets the local root volume with mount root "/" and no network voice', () => {
    expect(resolveSearchTargetVolume([ROOT, NAS], 'root')).toEqual({
      volumeId: 'root',
      mountRoot: '/',
      isNetwork: false,
    })
  ...
  it('targets a network (SMB) volume: its id, its mount root, and the network voice', () => {
    ...
    expect(resolveSearchTargetVolume([ROOT, NAS], 'smb-naspi')).toEqual({
      volumeId: 'smb-naspi',
  ...
  it('targets a local attached (USB) volume with its mount root but the local voice', () => {
    ...
    expect(resolveSearchTargetVolume([ROOT, USB], 'volumesusb')).toEqual({
      volumeId: 'volumesusb',
  ...
  it('falls back to the local root when the focused volume is not in the list', () => {
    ...
    expect(resolveSearchTargetVolume([ROOT, NAS], 'search-results')).toEqual({
      volumeId: 'root',
      mountRoot: '/',
      isNetwork: false,
    })
  ...
  it('falls back to the local root when the volume list is empty', () => {
    expect(resolveSearchTargetVolume([], 'smb-naspi')).toEqual({
      volumeId: 'root',
      mountRoot: '/',
      isNetwork: false,
    })
