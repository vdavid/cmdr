...
(`file-explorer/pane/editor-open.ts`).
...
- **`text-editor-setting.ts`**: reads and writes `behavior.textEditorApp`, plus the Settings deep link.
...
- **One toast id, `text-editor`, dismissed before every add**, ❌ never a one-slot `toastGroup`: a group full of
  persistent toasts drops the incoming one (`open-terminal/DETAILS.md` § The toasts). The pane guard's `notOnThisMac`
...
- **Read the choice before the first `await`** in `openFileInEditor`: two dispatch tests assert the `openInEditor` call
  synchronously after the key press.

What each outcome does, why a removed app falls back, the toasts, and testing: `DETAILS.md`.
