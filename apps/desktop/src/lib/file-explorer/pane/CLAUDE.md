...
- **Guard logic branches on `VolumeCapabilities`, ❌ never volume-id strings** and ❌ never a backend-sourced KIND:
  an un-upgraded SMB share is served by a local one.
...
- **Two archive path predicates, ❌ not swappable**: `pathCrossesArchiveBoundary` (at-or-inside) asks about a PANE
  path, `pathInsideArchive` (strictly inside) about a site acting ON one.
...
- **Each first-run-layout guardrail looks like a tidy-up. ❌ Never "simplify" one away** (`DETAILS.md` § First-run
  pane layout).
...
- **The `network` pane is the SERVERS HUB** (`NetworkMountView`): it owns its MCP push, so `pane-mcp-sync` skips it,
  and ❗ its NAME is spelled in four places (`../network/DETAILS.md` § Gotchas).
- **ONE typed state renders every remote wait** (`remote-connect-state.ts` + `RemoteConnectView`): `place-connect`
  gates on CONNECTION STATE, `device-connect` on `deviceReadiness`, both in FRONT of the kind chain. ❌ No second
  renderer, no inert affordance. ❗ A phone's dial HOLDS the listing (`holdsListing`), or the backend dials again and
  Cancel misses.
