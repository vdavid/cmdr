...
context menu, or click-to-rename (800 ms hold on the name). Operates on the cursor item only; selection is untouched.
...
- **A bare arrow chains the rename to the next row; four things inside that step fail silently.** The save fires BEFORE
