...
export const commands = {
   ...
   *  run on the IPC main thread, which has no Tokio reactor, so `spawn` panicked
   ...
   *  2. Start the MTP hotplug watcher. MTP is opt-in per device, so the
   ...
   *  "Cmdr wants to find devices on local networks" prompt, so we defer to the latest reasonable
 ...
 *  primitives via `#[serde(untagged)]`, so TS sees `boolean | number | string`.
 ...
 *  `Disconnected` means an SmbVolume exists but its smb2 session is broken;
 *  the frontend reconnect manager owns the recovery cycle.
 *
