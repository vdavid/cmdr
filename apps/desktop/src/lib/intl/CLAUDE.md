...
- `messages.svelte.ts`: the runtime: `t()` (catalog + ICU), `getMessage()` (raw), `setLocale()`, `availableLocales()`
  ...
  `document-language.ts`: the `<html lang>` write. `locale-display-names.ts`: the picker's row labels. `Trans.svelte`:
...
- **The resolver loads ALL locale dirs (`messages/*/*.json`) by dir tag, then falls back per key** to `en`.
  `screenshots/` is NOT a locale: keep every glob/gate excluding it, or it becomes a fake language.
...
- **Picker labels take the WHOLE shipped list**: `zh` reads `简体中文` only because `zh-Hant` ships. ❌ Never decorate
  unconditionally ("Deutsch (Lateinisch)"); keep every row distinct.
- **Both locale answers come from Rust, ❌ never from the webview tag**, which drops the region override. Go through
  ...
  `routes/window-route-coverage.test.ts`). `watchSystemLocales()` takes ONE subscriber per window (the first adopts):
  hang new reactions off it. Depth: `DETAILS.md`, `apps/desktop/src-tauri/src/intl/DETAILS.md`.
