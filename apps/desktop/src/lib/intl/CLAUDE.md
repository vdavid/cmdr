...
- `messages.svelte.ts`: the runtime: `t()` (catalog + ICU), `getMessage()` (raw), `setLocale()`, `availableLocales()`
  (the Language picker), the catalog map, BCP-47 resolver, locale-version rune, and compiled-message cache.
...
- **The resolver loads ALL locale dirs (`messages/*/*.json`) by dir tag, BCP-47 fallback** (locale → base → `en` → key).
  `screenshots/` sits among them and is NOT a locale: keep every glob/gate change excluding it, or it shows up as a fake
...
- **Both locale answers come from Rust, ❌ never from the webview tag**, which exposes ONE language tag and drops the
  ...
  the first adopts, so a second never fires — hang new reactions off it. What each mistake costs, the walk, the guard,
  and the composition: `DETAILS.md` and `apps/desktop/src-tauri/src/intl/DETAILS.md`.
...
- **Catalog messages double apostrophes (`''`).** ICU reads a lone `'` before `{`/`<`/`#` as an escape and swallows what
  follows; `''` always collapses to `'`, so double it everywhere.
...
- **Format ONLY through this layer + `$lib/settings/format-utils`**: `formatInteger`/`formatNumber` for counts,
  `formatSizeForDisplay` for sizes, `formatDateForDisplay` for dates. Never hardcode a locale or build an `Intl`
...
- **Both readers stay SSR-safe** (no `window`/DOM, never throw: the Node pass and viewer window call them) **and
  uncached** (their formatters are cached; a cache here would hide a switch).
- **Keep `Intl` formatters memoized by (locale, options).** They run per-visible-entry in render AND in the
  column-measurement fold, so per-call construction regresses scroll on big directories. `DETAILS.md`.
...
formatting tag, the en-US triad change): `DETAILS.md`.
