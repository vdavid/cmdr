...
- **`@key` metadata is ARB-style sibling entries** (`@transfer.trash`), holding `description` + `screenshot`. The
  runtime and codegen strip every `@`-prefixed entry, so it never reaches `format()`. Keep a `@key` twin in sync when
  you rename a key.
