...
- `screenshots/coverage-report.md`: **tracked**, regenerated by the coupler. Per area: how many keys are coupled vs not,
  with uncoupled keys split into `dynamic-only` (runtime-built `errors.*` keys no static surface can name) and
  `not driven` (a surface the driver doesn't visit yet). Coverage is partial until the driver covers the full inventory.
- The `@key.screenshot` refs in `en/*.json`: **tracked.** Written line-surgically (only that field changes; the coupler
  has a value-safety test).

The coupler is idempotent (a re-run with the same report is a byte-for-byte no-op) and assigns each key its FIRST
surface in the report's order (surfaces ordered narrow-to-broad, so the most-specific surface wins). The warn-only
