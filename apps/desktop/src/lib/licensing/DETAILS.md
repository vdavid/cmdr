...
- **Package links are `<button>`s, not `<a href>`s.** They open the system browser via the opener plugin rather than
  navigating the webview, and a dynamic `href` also trips `svelte/no-navigation-without-resolve`.
...
a11y test (`a11y-coverage` requires one per `lib/` component).
