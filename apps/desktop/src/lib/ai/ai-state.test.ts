...
vi.mock('$lib/settings', () => ({
  getSetting: vi.fn((key: string) => {
    if (key === 'ai.provider') return 'local'
    return undefined
  }),
}))

import { getAiStatus, getAiModelInfo, cancelAiDownload } from '$lib/tauri-commands'
import { getAiState, initAiState, handleCancel, handleGotIt, resetForTesting } from './ai-state.svelte'
...
describe('ai-state', () => {
  ...
  describe('initAiState', () => {
    it('stays hidden when status is offer (wizard owns AI consent now)', async () => {
  ...
  describe('handleCancel', () => {
    it('calls cancelAiDownload and hides notification', async () => {
      vi.mocked(getAiStatus).mockResolvedValue('available')
      vi.mocked(getAiModelInfo).mockResolvedValue(mockModelInfo)
