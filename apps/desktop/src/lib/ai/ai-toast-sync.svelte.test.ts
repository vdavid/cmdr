 ...
 * toast shows again, and other state transitions (ready, etc.) ignore it.
 *
...
describe('ai-toast-sync', () => {
  ...
  it('shows the toast again on a fresh download run after dismissal', () => {
    ...
    // The wizard's local-pick starts a fresh download: the flag clears, then `notificationState`
    // flips back through `'hidden'` (cancel/reset path) and into `'downloading'` again.
    ...
    // 1 from the first downloading, 1 from the new run after reset. The `'hidden'` transition
    // dismisses the toast rather than adding it.
