...
| `DownloadToastContent.svelte`            | In-app toast: title with filename + size, optional subdir line, two snapshotted shortcut hints (in-app `⌘J` and global `⌃⌥⌘J`, each a literal `ShortcutChip`), `GlobalShortcutAnimation` for the default global combo, and a right-aligned `Button` row (secondary "Stop showing these" + primary "Jump to file" at the far right). Collapsible: a chevron toggles between the full teaching view and a compact one-line summary; the choice persists for the NEXT toast. Auto-hides (transient, 10s) and wider than default (`widthPx`) so the animation reads. |
| `downloads-toast-collapsed.ts`           | Getter/setter for the hidden `behavior.fileSystemWatching.downloadsToastCollapsed` setting. The bridge reads it to seed each new toast; the toast's chevron writes it. No Settings UI.                                                                                                                                                                                                                                                                                                                                                                           |
| `download-toast-shortcuts.ts`            | Pure `buildShortcutSummary(shortcutHint, globalBinding)` → `{ inApp, global }` (nullable). Maps the two snapshotted bindings to the chips the collapsed summary line renders. Unit-tested in isolation.                                                                                                                                                                                                                                                                                                                                                          |
