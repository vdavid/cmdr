...
import { NO_WALKED_GROUND, isPathAffectedByWalk, walkedBranches, wholeVolumeWalked } from './walked-ground'

describe('isPathAffectedByWalk', () => {
  it('flags a row inside the branch being walked', () => {
    const ground = walkedBranches(['/Users/someone/Downloads'])
  ...
  it('flags every row while a run takes the volume whole', () => {
    // A full rebuild and every network scan walk the drive in one go and
    // announce no branches, so their whole-volume answer is the honest one.
    expect(isPathAffectedByWalk(wholeVolumeWalked(), '/anywhere/at/all')).toBe(true)
  })

  it('flags nothing when no walk is running', () => {
    expect(isPathAffectedByWalk(NO_WALKED_GROUND, '/Users/someone/Downloads')).toBe(false)
    expect(isPathAffectedByWalk(walkedBranches([]), '/Users/someone/Downloads')).toBe(false)
  })

  it('checks every announced branch, not only the first', () => {
    const ground = walkedBranches(['/opt/one', '/opt/two'])

    expect(isPathAffectedByWalk(ground, '/opt/two/deep')).toBe(true)
  })

  it('ignores a trailing slash on either side', () => {
    expect(isPathAffectedByWalk(walkedBranches(['/Users/someone/Downloads/']), '/Users/someone/Downloads/big')).toBe(
      true,
    )
    expect(isPathAffectedByWalk(walkedBranches(['/Users/someone/Downloads']), '/Users/someone/Downloads/')).toBe(true)
  })
})
