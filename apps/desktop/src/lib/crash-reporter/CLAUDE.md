...
The next-launch "Cmdr quit unexpectedly" surface: a dialog when the user hasn't opted in (or the app looks stuck in a
crash loop), a toast when they have. Capture, sanitizing, and upload are all backend (`src-tauri/src/crash_reporter/`);
this side only decides which surface to show and collects the user's choices.
