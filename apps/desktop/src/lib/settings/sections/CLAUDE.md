...
| `KeyboardShortcutsSection.svelte`  | `Keyboard shortcuts`: special (non-registry) section, renders the shortcut table from `shortcuts.json`, plus a bespoke `Global` group hosting `lib/downloads/GlobalShortcutRow.svelte` (the reveal hotkey, marked `(global)`, binding stored in `settings.json` not `shortcuts.json`)                                                                                                                                                        |
...
### Global reveal hotkey: on/off here, combo edited in Keyboard shortcuts

The "Reveal latest download" sub-group is a plain on/off `Switch`. The combo is edited under `Keyboard shortcuts`
...
rebinds. Both surfaces call the `set_global_reveal_shortcut` IPC on change for live-apply.
