...
- **A stall drops the ETA and says why** (`transfer-stall.ts`): the BACKEND classifies, this side owns the threshold. ❌
  Never infer a stall from event timing, since a wedge emits no events at all. DETAILS § "The stalled-transfer notice".
- **A cancel's reversal always drains its bar to zero, so the TOAST is what says what actually stayed**
...
- **"Couldn't find out" is its own state in BOTH pre-confirm checks, ❌ never silence.** Rendering nothing is what a
  CLEAN destination renders, and this feeds an overwrite decision. DETAILS § "When the dialog can't find out".
