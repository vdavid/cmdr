...
- `QueueRow.svelte`: one operation row. Line 1 is chrome (select, type icon, source→dest, status, pause/resume +
  cancel); line 2 is the shared `../TransferProgressReadout.svelte`, compact. Shell: `routes/queue/+page.svelte`.
...
- **Progress is DEFINED with the copy dialog, not here.** Speed and ETA are the backend's; the ETA goes through
  ...
  (rendering the raw value here while the dialog smoothed it showed one operation as "8m 12s" in one window and "5m 46s"
  in the other). The bars, amounts, percents, rates, and time left are `../TransferProgressReadout.svelte`, whose
  fixed-width columns set `queue-window.ts`'s `MIN_WIDTH`: narrow past that and the bars vanish, so the two move
  together.
...
- **Rows cover copy/move/delete/trash AND the instant ops `rename` / `create_folder` / `create_file`.** Instant ops emit
  ...
  not `createFolder`), or they fall silently to the `trash-2` / "Working" fallbacks; the mapping is the pure
  `operation-icon.ts`.
...
- **Cancel keeps partials, always (rollback = false).** Per-row Cancel and "Cancel selected" both map to
  ...
  running/paused op stops keeping copied files. That's why `capabilities/queue.json` DROPS `dialog:allow-ask` and
