...
describe('transfer progress dialog chrome (en)', () => {
  ...
  it('resolves the conflict comparison labels and annotations', () => {
    ...
    expect(tString('fileOperations.transferProgress.newFolderLabel')).toBe('Incoming folder:')
    expect(tString('fileOperations.transferProgress.newFileLabel')).toBe('Incoming file:')
    expect(tString('fileOperations.transferProgress.newLabel')).toBe('Incoming:')
