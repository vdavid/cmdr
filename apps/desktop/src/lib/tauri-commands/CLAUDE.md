...
`tags.ts`, `updates.ts`, `debug.ts`, plus mostly-event modules (several also carry a few commands): `indexing.ts`,
`ai.ts`, `appearance.ts`, `menu-events.ts`, `directory-watcher.ts`, `native-drag.ts`, `quick-look.ts`, `downloads.ts`,
`restricted-paths.ts`, `dialog-events.ts`. `ipc-types.ts` holds `TimedOut<T>` and the last-resort `throwIpcError`.
DETAILS.md has the per-file contents and the "where to put a new command" routing map.
