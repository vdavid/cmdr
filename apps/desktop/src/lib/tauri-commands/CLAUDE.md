...
| `dialog-events.ts`     | Window-management events: `onExecuteCommand` + `emitExecuteCommand` (the unified menu/cross-window relay), the MCP `dialog` lifecycle (`on{Open,Focus,Close}Settings`/`…FileViewer`/`…About`/`…Confirmation`, `onCloseAllFileViewers`, `onMcpSettingsClose`), `onViewerWordWrapToggled`, `onPersistRestrictedSetting` |
