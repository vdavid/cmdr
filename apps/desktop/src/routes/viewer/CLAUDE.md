...
- **`closeWindow()` uses `closeSelfWindow()` (backend hide + 100 ms); `canClose` and `windowReady` flip via
  `setTimeout(0)`, never rAF.** Each dodges a different failure: stalled IPC in other webviews, a macOS WebKit segfault
  mid-teardown, starved rAF in unfocused E2E windows. (§ Gotchas; `$lib/child-window-close`)
