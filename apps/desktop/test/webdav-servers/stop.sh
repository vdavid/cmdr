#!/bin/bash
# Stop the WebDAV fixture servers.
set -e
...
echo "Releasing the manual WebDAV lease (the stack downs only at zero holders)..."
released=false
if command -v go &> /dev/null; then
    if (cd "$REPO_ROOT/scripts/check" && go run ./stack-lease release webdav manual); then
        released=true
    else
        echo "WARN: WebDAV lease helper failed; falling back to a direct 'compose down'." >&2
    fi
else
