...
Playwright E2E tests for Cmdr, using tauri-playwright in Tauri mode: commands inject into the real Tauri webview via
Unix socket. The same tests run on macOS (native) and Linux (Docker); platform differences (Ctrl vs Meta) are handled by
the `CTRL_OR_META` constant in `helpers.ts`. For the architecture diagram, per-spec file table, run recipes,
multi-window patterns, and decisions, see [DETAILS.md](DETAILS.md).
...
- **Run only the spec you're iterating on.** The full suite takes ~10 min and a broken test takes the app down with it,
  cascading connection-error failures into later specs. DETAILS.md § "Running a single spec" has the exact invocations
  (the `pnpm` script hardcodes `--project tauri`, so a positional spec path collides; use `npx playwright test` for file
  filters).
- **`npx playwright test` alone fails with `ECONNREFUSED`.** The suite doesn't launch the app; it connects to an
  ...
  manually first. When manual, ALWAYS pair the launch with `; pkill -f 'target.*Cmdr'` (`;`, not `&&`, so cleanup runs
  even on failure): nothing else stops the main process, so leaks pile up fast.
...
- **Bare `await pollUntil(...)` is silent on timeout** (returns `false`, doesn't throw), so the test passes green even
  ...
  `pollActiveMode`, etc.). The `bare-poll` fast-lane check flags these; opt out only for genuine best-effort cleanups
  with `// allowed-bare-poll: <reason>`.
- **Exercise viewer + settings through the production multi-window flow**, not by routing the main window to `/viewer`
  ...
  `openViewerWindow` / `openSettingsWindowViaProd` / `closeScopedWindow` and interact via the scoped page. A scoped page
  that can't call a Tauri command is a REAL bug (production hits the same wall): fix the capability or use a permitted
  command, don't route around it.
...
- **Frontend `tauri-plugin-store` stores read your REAL store files unless redirected.** The checker launches the
  ...
  data; a locally-flipped setting then leaks in (passes in CI, fails locally). `getStore()` resolves through
  `resolveStorePath(name)`, which a `CMDR_DATA_DIR` instance redirects. If a persisted-UI-state spec fails locally but
  passes in CI, suspect a stale value in your real store.
