...
- **❌ Never `keyboard.press('Escape')`** to close an overlay: under Linux Xvfb it can vanish as an opaque timeout. Use
  `dismissOverlay` / `expectAndDismissToast` / `dismissAllToasts`, and no defensive double-Escape in `beforeEach`.
- **Two ways a helper claims success it never got.** Bare `await pollUntil(...)` returns `false` on timeout, so the test
  ...
  actionability: dialogs disable their buttons for the previous answer's whole IPC round trip, so back-to-back answers
  race it and a loaded runner loses. One lost answer wedged 196 tests.
...
- **Need the other pane focused? Click its `.file-pane` and read `.is-focused` back.** ❌ Never dispatch the
  `pane.switch` TOGGLE at it, and never steer by `cmdr://state`'s `focused:`: a toggle on that stale mirror lands the
  action in the wrong pane. DETAILS § "Claiming a pane's focus inside a spec".
- **One global `afterEach` guards TWO leaks: UI artifacts, and a dirty `left/` + `right/`.** A mutating spec restores
  the tree (`restoreFixtureTree(getFixtureRoot())`) or the guard names it. ❌ Don't relax it. **Still holding an op?
  Drain it BEFORE the restore, in ONE hook** (`afterEach`s run in DECLARATION order): a restore under a live op deletes
  its source, and the retained `SourceNotFound` poisons the next one.
...
- **`emitBackendEvent` drives UI off a synthetic backend event** rather than racing real work. The app is SHARED: emit
  the terminal event that clears it (test AND `afterEach`), under an id nothing real claims. DETAILS § "Synthetic
  backend events".
- **The marketing capture (`marketing-shots.spec.ts`) photographs real folders, with NO fixture tree.** ❌ Never point
  ...
  through `shoot()`, and ❗ needs the machine left alone, so say both before starting one. Full contract: DETAILS.
