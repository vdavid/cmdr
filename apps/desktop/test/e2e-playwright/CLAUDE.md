...
- **Run only the spec you're iterating on.** The full suite takes ~10 min and a broken test takes the app down with it,
  cascading connection-error failures into later specs. The `pnpm test:e2e:playwright` script pins `--project tauri`, so
  a positional spec path collides ("Project not found"): use `npx playwright test` for file filters, the `pnpm` script
  for `--grep`. Exact invocations: DETAILS.md § "Running a single spec".
- **`npx playwright test` alone fails with `ECONNREFUSED`.** The suite doesn't launch the app; it connects to a running
  one over the socket. Use `pnpm check desktop-e2e-playwright` (full lifecycle), or start the app manually and ALWAYS
  pair the launch with `; pkill -f 'target.*Cmdr'` (`;`, not `&&`, so cleanup runs even on failure): nothing else stops
