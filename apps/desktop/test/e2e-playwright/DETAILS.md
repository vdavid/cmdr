...
- **`helpers/`**: Themed helper modules behind the `helpers.ts` re-export: `core.ts` (constants, `sleep`/`pollUntil`,
  DOM queries, key/drop dispatch, size+count), `app-lifecycle.ts` (`ensureAppReady`), `cursor.ts`,
  `overlays-and-dialogs.ts`, `windows.ts`, `navigation.ts`. Submodules depend on `core.ts`, never on each other (except
...
- **`conflict-dialog-matrix.spec.ts`**: 18 tests: per-file conflict-dialog state machine across clash types (file/folder
  × file/folder) × resolutions (Skip/Rename/Overwrite + apply-to-all), the two-bucket `ApplyToAll` latch (normal vs
  file→folder), and cross-type Overwrite atomicity (no `.cmdr-temp` survivors)
- **`file-watching.spec.ts`**: 11 tests: external CRUD, batch/threshold, cross-pane, dedup, hidden
- **`focus-trap.spec.ts`**: 4 tests: dialog focus trapping (`use:trapFocus`) — palette Tab swallow, focusin leak guard
  pull-back, Escape fallback with leaked focus, ModalDialog Tab wrap at both ends. Regression anchor for the palette
  Tab-Tab keyboard lockout
- **`indexing.spec.ts`**: 3 tests: directory sizes from index, size updates on create/delete
- **`settings.spec.ts`**: 9 tests: sidebar order, sub/top-level navigation + content wiring, summary cards, Advanced
  auto-generated rows, search, ArrowDown nav, Escape-to-close
...
- **`viewer-encoding-picker.spec.ts`**: 1 test: switching encoding from UTF-16 LE to UTF-8 on a 6 MB file keeps the
  viewport interactive during the background rebuild.
- **`viewer-tail.spec.ts`**: 1 test: enabling tail mode auto-extends the viewport when the file grows on disk. Uses
  `fs.appendFile` to drive the FSEvents pipeline end to end. `test.describe.configure({ timeout: 30000 })` to absorb
  FSEvents debounce + BE coalescer.
- **`viewer-wordwrap-persistence.spec.ts`**: 1 test: word wrap toggled in one viewer session is on in the next.
...
- **`mtp.spec.ts`**: MTP E2E tests: volume selection, navigation, file ops, large file transfer via virtual device. Uses
  ...
  feature.
- **`mtp-conflicts.spec.ts`**: MTP conflict resolution: cross-volume move (MTP↔local) and same-volume move (MTP→MTP)
  with overwrite/skip policies. Requires `virtual-mtp` feature.
- **`mtp-same-volume-move-merge.spec.ts`**: Same-volume rename-merge fast path: MTP→MTP folder move auto-merges (no
  folder prompt), a clashing file inside follows the file policy (prompts under Stop), and a dest-only file survives.
  MTP-backed (uses the real `move_within_same_volume` rename-merge), so it lives on the dedicated MTP shard. Requires
  `virtual-mtp` feature.
- **`drag-drop-entry.spec.ts`**: Programmatic drop entry, local-only: a local→local drop opens the copy dialog with
  correct counters; toggling that dialog to Move SURVIVES the counters (regression pin — a local→local move must keep
  the deep scan, not zero the tallies). Drives `triggerFileDrop` (see below). Non-MTP shard.
- **`mtp-drag-drop-entry.spec.ts`**: Programmatic drop entry across MTP: dropping onto the read-only SD Card shows the
  exact "Read-only device" alert with no transfer dialog (a drop must hit the same read-only guard F5 does); MTP→local
  and local→MTP drops fill the counters from the resolved source volume (a wrong source volume id makes the preview
  report zeros). MTP shard. Requires `virtual-mtp`.
- **`smb.spec.ts`**: SMB E2E tests: virtual host discovery (14 hosts), share listing, mounting, cross-storage copy,
  50-share enumeration, unicode share rendering. Uses `e2e-shared/smb-fixtures.ts` and smb2's consumer Docker
  containers. Requires `smb-e2e` feature + Docker.
