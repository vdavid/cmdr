...
| ---------------------------------- | ------------------------- | --------------------------------- | ----------------------------------------------------------------- |
| **Playwright** (`e2e-playwright/`) | Playwright + tauri-plugin | macOS (native) and Linux (Docker) | Full app: dialogs, keyboard nav, file ops, settings, viewer, a11y |
...
the Tauri binary with `--features playwright-e2e` inside Docker, launches it, and runs the Playwright tests.
