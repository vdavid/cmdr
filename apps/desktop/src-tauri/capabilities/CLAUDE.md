...
- **`playwright:default` can't go in `default.json`.** Its permission schemas only exist under the `playwright-e2e`
  feature, so listing it there breaks normal builds (Tauri rejects unknown permissions). `build.rs` generates a separate
  `playwright.json` only when the feature is active.
