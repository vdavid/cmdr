...
- `shortcuts.json`: read-only Keyboard shortcuts help window. `debug.json`: dev-only debug window, self-contained with
  `core:default`. `playwright.json`: auto-generated by `build.rs` under the `playwright-e2e` feature (gitignored).
- `queue.json`: transfer-queue window. Same window perms as `settings.json` but WITHOUT `store:default` (no persistence
  in v1) or `dialog:allow-ask` (keep-partials cancel needs no confirm). Lists only `"queue"`.
