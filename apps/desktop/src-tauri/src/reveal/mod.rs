...
//! `NSFileViewer` global default); this module owns what arrives once it's on — macOS
//! delivers the reveal as an open-documents Apple Event, which Tauri surfaces as
//! `RunEvent::Opened`, handled in `app_lifecycle.rs`.
//!
//! The pane move itself is not ours: it's `crate::mcp::go_to_in_focused_pane`, the same
//! navigate-then-cursor primitive `go_to_latest_download` uses. `DETAILS.md` says why it
//! lives there.
...
fn spawn_delivery<R: Runtime>(app: &AppHandle<R>, paths: Vec<PathBuf>) {
    let app = app.clone();
