...
labels with user-customized shortcuts, and enables/disables items based on window focus context.
...
- **Accelerator changes go remove/recreate/reinsert, not in-place.** Tauri has no `set_accelerator()`.
  ...
  reference and positional index per updatable item for exactly this.
