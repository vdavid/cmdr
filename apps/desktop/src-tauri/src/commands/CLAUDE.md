...
One file per domain (`network.rs`, `sftp.rs`, `webdav.rs`, `servers.rs` (the protocol-agnostic facade over those
three), `mtp.rs`, `clipboard.rs`, …), plus `mod.rs` (re-exports + platform gates), `util.rs` (timeouts and budgets),
`file_system/`, `media_index/`, and `importance.rs`. AI and space-poller commands register from their own modules; the
index subsystems are the reverse, since they can't carry `tauri::`. Inventory and rationale: `DETAILS.md`; `sftp.rs`'s
frontend contract is `crates/cmdr-sftp/DETAILS.md` § "Connecting from the frontend"; `webdav.rs`'s,
...
- **`delete_files` and `rename_file` accept a `volume_id`** and behave differently off the root volume. DETAILS.
- **The drag commands need the main thread, and locality comes from `Volume::paths_are_os_visible()`, ❌ never
  `supports_local_fs_access()`.** DETAILS.
- **⌘W: `CLOSE_TAB_ID` is the one menu item NOT disabled when the main window loses focus.** `menu/DETAILS.md`.

Inventory, decisions, and the detach rationale: `DETAILS.md`. Read it before non-trivial work here.
