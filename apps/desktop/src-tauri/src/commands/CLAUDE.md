...
| `error_reporter.rs` | Error reports (Flow A) | `prepare_error_report_preview`, `send_prepared_error_report`. Two-step so the preview dialog is deterministic without shipping the full bundle through IPC twice. Delegates to `error_reporter` module. Upload is skipped in dev/CI. |
