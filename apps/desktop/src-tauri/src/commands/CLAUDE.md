...
- **`create_directory_core` / `create_file_core` error on an unregistered volume; NO `std::fs` fallback.** "root" and
  ...
  A bare `std::fs` fallback has no timeout and breaks the "every FS-touching command is timed" contract on a hung mount;
  don't re-add it. Unit tests register a real local "root" via `ensure_root_volume()` (never `init_volume_manager`).
...
- **`delete_files` and `rename_file` accept `volume_id`.** Non-root → `delete_files` uses the volume-aware delete and
  ...
  for MTP and skips permission checks for non-root. `rename_file` calls `notify_mutation` after success.
