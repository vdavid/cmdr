...
use crate::error_reporter::{self, BundleKind, BundleManifest};
use serde::Serialize;
...
pub async fn prepare_error_report_preview(
    ...
    let bundle = error_reporter::build_bundle(&app, BundleKind::User, note)?;
    Ok(PreviewPayload {
        id: bundle.id,
        size_bytes: bundle.zip_bytes.len(),
...
pub async fn send_error_report(app: tauri::AppHandle, user_note: Option<String>) -> Result<SendResult, String> {
    let note = validate_user_note(user_note)?;
    let bundle = error_reporter::build_bundle(&app, BundleKind::User, note)?;
    let result = error_reporter::upload(bundle.zip_bytes, &bundle.manifest, ERROR_REPORT_URL).await?;
