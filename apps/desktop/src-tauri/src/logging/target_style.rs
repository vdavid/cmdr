...
/// Whether the terminal chain should emit ANSI at all: only when stderr is a real
/// terminal (a piped or redirected `pnpm dev` gets clean text) and the `NO_COLOR`
/// convention isn't asking us to stop.
