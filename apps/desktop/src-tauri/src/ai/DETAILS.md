...
- **The real-API provider smokes** (`client_real_{groq,fireworks,anthropic,openai}_test.rs`): `#[ignore]`-gated live calls through `AiBackend::remote`, one file per provider. They catch adapter-routing / auth / parse regressions the wiremock tests can't (it's what caught the Ollama-fallback bug above), plus the thing only a live call can see: a provider retiring the model we name. Each has a `<provider>-smoke` check lane (Go runner) that resolves the key from env or the sops `secret` helper and runs it with `--run-ignored only`, self-skipping without one. All are CI-only, so locally they run on demand (`pnpm check groq-smoke`); CI: `slow-checks.yml` has a step each. Mechanics and the CI-only rationale: `scripts/check/checks/DETAILS.md`.
  - **Why four and not one.** Groq alone only ever notices Groq's schedule. The set covers each distinct code path once: Groq and Fireworks the forced `openai::` namespace (Fireworks additionally the `accounts/…/models/…` id shape, which must survive namespacing and `Url::join` intact); Anthropic the one non-OpenAI wire format we ship; OpenAI all three of its legs (chat-completions, the Responses API, and the `o*` chat-completions reasoning models that reject `temperature`) — so those three tests are deliberately NOT interchangeable, and `client.rs`'s `the_two_openai_smoke_models_cover_different_branches` fails if a swap collapses them.
  - **Gemini is the uncovered adapter.** It's the second native protocol we ship and no lane calls it, because there's no Google AI key in the sops store. Closing that gap is worth more than any further OpenAI-compatible host, which Groq and Fireworks already cover between them.
