...
- **One writer thread, one cross-volume DB, NO per-volume registry.** `record_items` BLOCKS under backpressure
  (lossless); a DB error drops that ONE row. The completeness check then degrades the op rather than under-reversing it
  silently.
...
- **The writer stores terminal state; it does NOT compute eligibility.** That reasoning lives in `capture.rs`. A driver
  can also NOTE a reason the rows can't express (`note_not_rollbackable`, e.g. a directory merge); a note beats the
  per-kind rule.
...
- **Op ids are UUIDv7 (`new_operation_id`), never v4.** The clock is whole SECONDS, so same-second ops tie and every
  ordered read falls back to `op_id`; v7 sorts chronologically, v4 is a coin flip. `read_inverse_op` leans on it, not
  just the feed.
