...
- **A match needs the same language AND the same script.** `zh-Hant-TW` must not open the Simplified `zh` catalog: for
  ...
  variants: `pt-PT` → `pt` and `en-GB` → `en` are WANTED (dialect friction is a papercut, an unreadable script a wall).
...
The walk, the script data, the specificity rule, the composition, the live-change path, and the native-string table:
`DETAILS.md`. Frontend: `apps/desktop/src/lib/intl/`.
