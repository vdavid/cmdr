...
///
/// Write these WITHOUT inline flags. `keyword_mapping::merge_keyword_and_type` is the
/// single place that prefixes `(?is)` onto whatever reaches the query, so a pattern
/// carrying its own would be the second opinion this arrangement exists to avoid.
/// The `s` matters to any pattern using `.*` (`screenshots`, and the `.*` the merge
/// itself inserts): a filename may contain a newline, and `.*` has to reach past it.
