...
| `prompt.rs` | `CLASSIFICATION_PROMPT` const + `build_classification_prompt()`. Substitutes `{TODAY}` at runtime. Instructs the LLM to emit `keywords / type / time / size / scope / exclude / folders / note / label` one-per-line |
