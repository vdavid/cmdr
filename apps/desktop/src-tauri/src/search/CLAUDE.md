...
- **The broad-query guard is per evaluator**: a refusing walk takes the whole RUN with it, ❌ never answering from the
  index alone.
- **Coverage is asked BEFORE the arena loads, and reloaded when a walk wrote behind it** (Decision 12). "Covered"
  promises the arena holds those rows; break it and the NEXT query silently returns fewer.
