...
- **A directory's size filter is applied BEFORE ranking**, from `dir_sizes_for`'s `dir_stats` map (its size isn't in
  the arena). ❌ Never after: that answers from a recency-ordered top-k, which is how `sizeMin: 50 GB` missed a 1.7 TB
  `~/Library`. ❌ Never fall back to "no map" on a read error — the engine reads that as "no filter". Count-only is
  then exact with no correction pass. `DETAILS.md` § Directory size filters.
