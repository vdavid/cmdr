...
- `tool_registry/`: single source for every AI-callable tool — one `mcp_tools!` table (`mod.rs`) generates the
  ...
  `executor/` (see `executor/CLAUDE.md`).
...
- **One registry, two consumer views (agent-spec D49/D59).** Each entry declares `consumers` (`ai_client`/`agent`) +
  ...
  `../agent/tools/CLAUDE.md`. DETAILS.md § The `Propose` tier.
...
- **Volume capacity/free comes from the space poller's CACHE, never a `statfs` here** (`volumes::space_summary`): that
  syscall blocks 30–120 s on a hung mount, and `cmdr://state` is read constantly. Unwatched volume ⇒ absent, never a
  guessed zero. To widen coverage, widen what the poller watches.
...
- **Interactive rebinds use bind-new-before-stop** (`rebind_interactive`, `BindMode::Exact`): a busy port leaves the
  ...
  Two stop flavors + token-clear-on-stop: `DETAILS.md`.
