...
- Cursor/Selection (3): `move_cursor` (honest errors: a missing filename or out-of-range index is a round-trip failure, never a false OK), `open_under_cursor`, `select` (index ranges, `all`, `count: 0` to clear, or `names: [...]` — the names mode is a round-trip that errors listing any names not in the listing)
- File operations (6): `copy`, `move`, `delete`, `mkdir`, `mkfile`, `refresh`. `copy`/`move`/`delete` fast-fail with the real cause when there's nothing to act on (no selection and the cursor is on `..`, or the pane shows no files) instead of a misleading ack timeout. `copy`/`move` accept optional `autoConfirm` (bool) and `onConflict` (`skip_all`|`overwrite_all`|`rename_all`). `onConflict` governs clashing **files only** — folders always merge (a source folder landing on a same-named dest folder merges into it; the policy then applies to the files inside). `delete` accepts optional `autoConfirm`. When `autoConfirm` is true, the dialog opens and immediately confirms.
