...
pub fn stop_mcp_server() {
        ...
        && let Some(handle) = guard.take()
    {
        handle.abort();
        MCP_ACTUAL_PORT.store(0, Ordering::Relaxed);
        // Clear the in-process token so `validate_token` fails closed and `get_mcp_token`
        // reports "not running".
        set_mcp_token(None);
        // Remove the port and token files so external readers see the stop. Logged but
        // ignored on failure: a stale file is just a leftover, not a correctness bug.
        if let Ok(mut slot) = port_file_dir_slot().lock()
