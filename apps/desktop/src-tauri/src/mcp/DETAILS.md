...
The classification is **sourced from the tool registry, not a separate string list** — this is the by-construction win. Each tool declares a `TokenGate` (`Open` / `Always` / `IfAutoConfirm` / `IfConfirmAction`) on its `mcp_tools!` entry in `tool_registry.rs`. `auth::tool_call_requires_token(method, params)` (a pure, unit-tested predicate) returns true iff `method == "tools/call"` and `tool_gate(name)` reports a gate whose `requires_token(arguments)` is true. Because the gate is a required field on every entry, a new destructive tool can't ship with the gate forgotten — and a structural test (`test_autoconfirm_tools_are_gated`) fails if a tool exposing `autoConfirm` is left `Open`, while a full-table set-equality test forces a conscious gate for any newly-added tool. `TokenGate::IfConfirmAction` reads the tool's own typed `action` enum (`open|focus|close|confirm`), not a message substring, so it's not a `no-string-matching` violation.
