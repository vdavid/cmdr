...
**Why.** Two reasons. First, coverage: the `select` tool already selects by names, index range, or all, and `ai_search` already runs a natural-language query — together they cover what the Selection dialog does programmatically, so a dialog-opener adds little. Second, cost: `open_search_dialog` is cheap only because the search dialog is a registered soft dialog (`search` in `ui/dialog-registry.ts`) that notifies the `SoftDialogTracker` on mount, so the handler acks on `SoftDialogAppeared("search")`. The Selection dialog is a thin `QueryDialog` wrapper with NO registered `SoftDialogId`, so mirroring the tool means first registering a `selection` soft dialog, wiring its mount/destroy notifications and an FE prefill listener, then adding the tool/handler/tests — well past the "cheap" bar this was gated on. Add it only if the Selection dialog gains soft-dialog registration for another reason.
