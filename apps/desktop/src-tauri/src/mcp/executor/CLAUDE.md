...
| -------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `GenerationAdvanced`                   | `PaneStateStore.generation` strictly greater than captured value | Pane mutators: `select`, `set_view_mode`, `sort`, `toggle_hidden`, `tab`, `nav_*`, auto-confirmed `copy`/`move`/`delete`, `dialog confirm`. NOT `refresh` (FE skips the push when listing is byte-identical). |
...
- **`refresh` is still fire-and-forget.** The FE skips the `update_*_pane_state` push when the new listing is byte-identical to the cached state — correct for state sync but no signal for `wait_for_ack`. Switching `refresh` to `mcp_round_trip` is the right follow-up but needs an FE change to emit `mcp-response` after every re-list. Less acute than the destructive tools, so it stays open. Search `TODO(mcp-ack):` for this and any similar cases.
