...
mcp_tools! {
    ...
    "inspect_file" => {
        desc: "Look inside files: metadata, the format the bytes really are, and per kind the content: a line window of text (any encoding), an archive's entries (a zip, tar, or 7z, or a path inside one), or an image's dimensions and camera data (then image_facts for what's in it). find searches text files. Up to 200 paths; every cut is reported.",
