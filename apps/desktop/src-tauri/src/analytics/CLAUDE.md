...
- **Dev/CI suppression.** `suppressed()` is true in debug builds and when `CI` is set, so neither pollutes production
  analytics. `CMDR_ANALYTICS_FORCE=1` overrides it, letting an integration test drive the loop against a localhost
  Worker.
...
- **Name events after the UI** (project rule): user-facing vocabulary (`pane_navigated`, `search_used`), categorical
  props (`volume_kind`, `mode`). Events are an OPEN set: adding one is a one-liner, and a count goes through
  `item_count_bucket`.
