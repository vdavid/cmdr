...
app SURVIVES goes out in-session via the error reporter's Flow B. Everything else: `error_reporter/`.
...
- **`mod.rs`**: hook install + body, crash file I/O, crash-loop detection, `AppFate` resolution. **`panic_courier.rs`**:
  ...
  can make. **`signal_handler.rs`**: the async-signal-safe SIGSEGV/SIGBUS/SIGABRT path and its raw file format.
  **`symbolicate.rs`**: next-launch symbolication of its addresses. Tests in `*_tests.rs` siblings.
- IPC in `commands/crash_reporter.rs` (`check_pending_crash_report`, dismiss, send). Frontend in
  `src/lib/crash-reporter/`, driven from `(main)/+layout.svelte` after settings load.
