...
`POST /error-report`. Two entry flows: Flow A (the user-initiated "Send error report" dialog) and Flow B (opt-in
auto-send on user-visible errors).
...
- `auto_dispatcher.rs`: Flow B (debounced auto-send). `breadcrumbs.rs`: bounded ring buffer of triage events.
  `report_error()` is `log_error!`'s body as a function, for the one caller holding someone else's backtrace (the panic
  courier). Everything else uses the MACRO.

## Must-knows

- **Use `log_error!` at every error-level site in the desktop crate.** Downgrade a recoverable or non-user-impacting
  failure to `log::warn!`: the error-level threshold IS the auto-report threshold. `pnpm check log-error-macro` fails
  on a raw `log::error!` outside the macro definition.
...
- **Only `BundleKind::Auto` notes get redacted; `User` notes ship verbatim.** An auto note comes from a raw error
  message the user never previews (often with `current_exe()` paths), so `redact_line_salted` scrubs it like a log
...
- **Never widen what we send.** No license keys, transaction/device IDs, raw paths, volume names, SMB creds, settings
  ...
  RAM/disk/index *sizes*, an UNLABELED per-volume size list. Add nothing naming a drive, path, or person.
- **Don't gate `upload()` on `cfg!(debug_assertions)`.** Debug builds DO upload (that's "Send error report" in dev);
  `buildMode: "debug"` makes the api server prefix the Discord title `[DEV]`. `upload` short-circuits only on the `CI`
  env and the `playwright-e2e` feature, so E2E reports can't flood the live channel.
- **The server uses the client-supplied `id` verbatim.** The R2 key's trailing UUID already guarantees uniqueness, and
  a mismatched id in preview vs. toast confuses users.
...
- **The auto-dispatcher does NOT flush on shutdown, by design.** A crash inside the 60s window drops the pending flush;
  panics are covered by `crash_reporter` instead, and soft errors restart a window next launch. Don't add a queue or
  on-disk persistence: the manual flow is the safety net.
...
- **No max-line-length assumption in the tail walker** (`CHUNK_SIZE` 64 KB): backtrace symbol metadata makes ~10 KB
  lines with no upper bound, and a long one spans chunks, accumulating in `pending`.
