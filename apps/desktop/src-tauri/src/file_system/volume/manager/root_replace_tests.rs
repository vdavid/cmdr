...
fn connected_at(root: &str) -> (VolumeManager, Arc<RetiringVolume>, impl Fn() -> bool) {
    ...
    manager.register(ID, Arc::clone(&volume) as Arc<dyn Volume>);
...
fn a_replaced_root_is_the_active_one() {
    ...
    let outcome = manager.replace_root_in_place(ID, successor(&volume, WIDE));
...
fn the_old_root_stops_resolving_after_a_replace() {
    ...
    let _ = manager.replace_root_in_place(ID, successor(&volume, NARROW));
...
fn replacing_a_root_in_place_retires_nobody() {
    ...
    let _ = manager.replace_root_in_place(ID, successor(&volume, WIDE));
...
fn a_replace_announces_the_volume_like_register_does() {
    ...
    let _ = registry.replace_root_in_place(ID, successor(&volume, WIDE));
...
fn replacing_the_root_of_an_unregistered_id_registers_nothing() {
    ...
    let outcome = manager.replace_root_in_place(ID, Arc::new(InMemoryVolume::new("stray").with_root(WIDE)));
