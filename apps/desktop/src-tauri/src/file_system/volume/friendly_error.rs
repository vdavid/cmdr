...
// ============================================================================
// Layer 1b: WriteOperationError → FriendlyError
// ============================================================================
...
pub fn friendly_from_write_error(err: &crate::file_system::write_operations::WriteOperationError) -> FriendlyError {
    ...
    match err {
        W::SourceNotFound { path } => FriendlyError {
            ...
            explanation: format!("The file or folder at `{path}` isn't there anymore — it may have been moved or deleted while Cmdr was working."),
        ...
        W::PermissionDenied { path, .. } => FriendlyError {
            ...
            suggestion: "Check folder permissions, or try a different destination. On macOS, you may need to grant **Full Disk Access** in System Settings > Privacy & Security.".into(),
            raw_detail,
            retry_hint: false,
        },
        W::InsufficientSpace {
        ...
        W::TrashNotSupported { path } => FriendlyError {
            ...
            explanation: format!("`{path}` is on a volume that doesn't have a trash (network shares, FAT-formatted drives, …)."),
        ...
        W::IoError { path, message } => FriendlyError {
            ...
            suggestion: "Try again. If it keeps happening, expand **Technical details** below to share the specifics.".into(),
            raw_detail,
            retry_hint: true,
        },
    }
...
// ============================================================================
// Empty-but-restricted volume hints
// ============================================================================
...
/// by TCC, and only at the volume root (the root being empty is the rarest
/// "really empty" case). The user gets a "Try again" button via `retry_hint: true`
/// so they can re-list once they've granted access.
///
