...
- **A site passing a path calls `VolumeManager::resolve(volume_id, path).await`, ❌ never `get(volume_id)`.** `resolve`
  routes a `.zip`-crossing path to a read-only `ArchiveVolume`, path UNCHANGED; it's async because a remote `.zip` probes
  the network. `resolve_local_only` is for the ONE caller that can't `.await`.
- **Watcher-pre-registered volumes go in via `register_if_absent`**, else the FSEvents watcher overwrites an
  `SmbVolume` with a `LocalPosixVolume`. Plain `register` is for explicit replacement at the SAME root: two roots
  claiming one ID keeps the incumbent, so registration order can't decide where a volume is rooted. `DETAILS.md`
  § "Key decisions".
- **A volume the registry REMOVES is retired** (`Volume::retirement`), so a backend's watcher and reconnect loop stand
  down. A replace deliberately doesn't: a re-root hands the id to a share that's still live. `DETAILS.md` § "Leaving the
  registry".
...
- **Capability flags default to the conservative answer** (`Err(NotSupported)` / `false`), so a new backend starts at
  ...
  `capabilities()` is a pure fold published to the frontend as DATA; ❌ never override it.
- **`lane_key()` is the operation manager's serialization key** (default = the volume root). Override it when several
  `Volume`s share one resource.
...
- **`LocalPosixVolume` has two fixed-order path hooks**, each silently serving the wrong directory when wrong:
  `resolve` (= `root_anchored`), then `.git` read delegation. `DETAILS.md` § "Git delegation hooks".
