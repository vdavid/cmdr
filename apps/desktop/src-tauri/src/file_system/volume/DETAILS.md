...
- `create_directory_all()`: reports `DirectoryCreation::{Created, AlreadyExisted}` for the LEAF. The copy driver skips its per-file destination conflict probe on `Created` (`transfer/DETAILS.md` § "Skipping the destination pre-check"), so an overriding backend must answer honestly and answer `AlreadyExisted` when unsure — including when it lost a create race.
