...
// ── Remote-backed archive over a virtual MTP device (M5) ────────────
//
// The MTP counterpart to the live-SMB archive tests: a zip living on a virtual
// MTP device browses and extracts through `MtpVolume::read_range`
// (GetPartialObject64), and a remote EDIT (pull → apply → upload → swap) commits
// through the device's delete-then-rename swap (MTP allows same-name siblings, so
// it must NOT attempt an atomic rename-overwrite). Requires `virtual-mtp`.
...
async fn connect_virtual_device_with_zip(root: &Path, zip_bytes: &[u8]) -> (String, u32) {
    ...
    // so the device's object tree picks it up (no watcher-timing dependency).
    std::fs::write(root.join("internal/bundle.zip"), zip_bytes).expect("seed zip on device");
    rescan_virtual_device();
