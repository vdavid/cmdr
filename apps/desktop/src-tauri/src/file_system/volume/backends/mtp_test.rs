...
// ── `Volume::delete` stops at one node ──────────────────────────
//
...
// `MtpConnectionManager` (`MtpDeleteScope`), and these tests pin both halves of
// the split.
...
/// cache-only). Returns the device id, storage id, the volume, and the storage's
/// backing dir.
#[cfg(feature = "virtual-mtp")]
...
async fn delete_refuses_a_non_empty_folder_with_a_typed_error() {
    ...
    assert!(
        matches!(outcome, Err(VolumeError::IoError { raw_os_error: Some(_), .. })),
