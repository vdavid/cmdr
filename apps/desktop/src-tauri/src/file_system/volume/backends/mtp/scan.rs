//! `MtpVolume`'s scan surface: the copy scan, its parent-grouped batch form, and
//! the destination conflict scan.
//!
//! An inherent impl rather than part of `volume_impl.rs`'s `impl Volume`, so the
//! trait file stays a readable index of the surface and the scan logic (the
//! parent-grouping and the watched-listing oracle) reads on its own. The trait
//! methods in `volume_impl.rs` are one-line delegates to these.

use super::{MtpVolume, map_mtp_error};
use crate::file_system::listing::FileEntry;
use crate::file_system::listing::caching::try_get_watched_listing;
use crate::file_system::volume::{BatchScanResult, CopyScanResult, ScanConflict, SourceItemInfo, Volume, VolumeError};
