...
- `volume.rs`: `GitPortalVolume`, the read-only `Volume` the resolve routes virtual paths to. `overlay.rs`: the
  ...
  helpers, which hand back numbers and ids on a typed `git_meta`, never words. `FriendlyGitError` is in `cmdr-fs`
...
- **Flipping the toggle must refresh open listings.** `set_show_virtual_git_portal` flips the atomic AND calls
  `watcher::refresh_all_virtual_listings_after_toggle`, across every volume; the atomic alone leaves stale children on
  screen.
...
- **The Size column carries a FACT, ❌ never a sentence.** Every virtual row sets `FileEntry.git_meta`
  ...
  its tooltip. A new row shape means a variant plus its two catalog keys.
