...
`OperationEventSink`. State, intent, cancel/rollback, ETA, the conflict mutex, the settle contract: `../CLAUDE.md`.
Frontend: `apps/desktop/src/lib/file-operations/transfer/CLAUDE.md`.
...
- **The merge invariant**: a merge never deletes or overwrites a dest file the source doesn't shadow — every policy,
  every backend, cancel/rollback/retry mid-merge included (`volume_merge_tests.rs`).
...
- **A cross-volume file write stages on `.cmdr-tmp-<uuid>` and takes its final name only after its last byte**: a
  ...
  carrying `state.liveness_token()` so it stays hidden.
- **Cross-volume file→file Overwrite is a safe-replace** (sibling temp + `finalize_safe_replace`); that temp is
  committed data, ❌ not a partial. Cross-type is delete-first.
...
- **Cross-FS move source-delete preserves Skipped sources and runs AFTER `flush_created_destinations`.** Same-volume
  move is a rename-merge with top-level hints only (`bytes_total = 0`), never a subtree walk.
...
- **Cross-volume copy parks/yields between chunks** (`CheckpointStream`): park in place, NO release/reopen; auto-yield
  ...
  write-yield (SMB only) is HARD-CAPPED — it holds a write handle the server reaps.
...
- **Every phase must announce itself** (`transfer_probe.rs`): a parked transfer is invisible to a stack sample. ❌ No
  `.await` on a transfer path without a phase around it. The probe is ALSO the UI's stall signal (a `TransferActivity`
  per progress event), so register on BOTH paths; ❌ never derive a stall from FE timing — a wedge emits NOTHING.
- **A transport blip retries the FILE** (`retry.rs`), ONLY inside `stream_pipe_file`: 3 attempts, bounded cancel-aware
  ...
  journal, and the milestone live above and happen once. Retryability is an exhaustive typed match.
- **The stall watchdog ACTS**: past `STALL_ABORT_AFTER` it ends a task's wait, which the retry treats as a blip. ❌
  Never for a park, pause, prompt, cancelling op, or single-shot write; re-armed per attempt.
- **Both progress paths report a file's HIGH-WATER bytes** (concurrent `fetch_max`, ❌ not `swap`; serial
  `leaf_high_water`), so a retry's restart neither double-counts nor walks the bar backwards.
