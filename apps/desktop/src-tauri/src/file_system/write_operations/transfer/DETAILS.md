...
### Skipping the destination pre-check

Before spawning each top-level source, the CONCURRENT driver awaits `dest_volume.get_metadata(dest_item_path)`: `Ok` means something already sits at that name, so conflict resolution runs. It is one round trip PER FILE, serialized on the driver, and no window width can overlap it — a batch of N files carries a hard floor of `N × RTT`. Measured against David's QNAP at 3.7 ms RTT: **2.378 s of a 3.224 s best run for 500 files, 74%** (`docs/notes/transfer-concurrency-window-bench-2026-08-02.md`).

**Decision**: skip that probe when, and only when, THIS operation created the destination directory (Phase 0.5's `create_directory_all` answered `DirectoryCreation::Created`).
**Why**: nothing the user already had can be inside a folder that didn't exist a moment ago, so every probe in it is a guaranteed miss. This is the same rule the deep-merge walker one level down has always used — `copy_directory_streaming` skips its destination listing for a level whose `create_directory` returned `Ok` and lists once for a level that answered `AlreadyExists` (see "Scan-as-you-merge" above). The top-level spawn loop was the one place still paying N round trips to learn what one fact already answered.

What it deliberately does NOT do:

- ❌ **A merge into a pre-existing directory keeps probing every source.** That is the only case where a conflict can genuinely exist, and it is the case a user hits copying into the other pane's current folder. Pinned by `volume_copy_precheck_tests.rs::a_merge_into_a_preexisting_directory_still_probes_every_source`, which also asserts the pre-existing file survives under Skip.
- ❌ **"Created by us" is not "exists and is empty".** A directory that already existed can gain an entry from another process between any two instants; one we created cannot have held anything BEFORE we made it. Only the second claim licenses skipping. Pinned by `an_empty_but_preexisting_destination_is_still_probed`. ❌ Never relax this into an emptiness check — the failure is silent, a conflict that would have prompted becomes an overwrite.
...
- **Scoped to the concurrent loop, so MTP is untouched by construction.** `MtpVolume::max_concurrent_ops()` is 1, so a phone always takes the serial driver. That matters: an MTP `get_metadata` lists the entire parent directory (~18 s for 1046 photos on a cold cache), so MTP wants its own decision about this, not this one. The serial path pays at most a couple of probes anyway (it runs for `< 3` sources or a window of 1).

**Residual risk, and it is not zero.** Another process can write into our new directory WHILE the batch runs — a NAS with another client, a sync daemon, the user in Finder. That file would be overwritten with no prompt. The window is one operation's duration rather than a stale up-front listing's, and it requires someone to target a folder Cmdr created seconds ago, but it is a real narrowing of the guarantee rather than a free win. It is the reason the general "list the destination once up front" alternative was NOT taken: that one misses a file arriving mid-batch in a folder the user has been using all along, which is far likelier.

**How often it fires, stated honestly.** Every top-level probe targets the SAME directory (`dest_path`), so this is all-or-nothing per operation: either the operation created that directory and skips every probe, or it didn't and pays every one. `TransferDialog` seeds the destination with the opposite pane's current folder, which by definition exists — so **the default F5 copy keeps every round trip and is exactly as fast as before**, and the win lands on "the user typed a destination folder that doesn't exist yet". Measured on a QNAP at 500 files: 3.462 s → 915 ms (3.78x) in the fresh case, recovering ~99% of the measured 2.565 s probe floor; 0% in the merge case, by construction. ❌ Don't describe this as "many-small network copies got faster"; it is "copies into a folder Cmdr creates got faster".

**Not taken, and worth knowing about.** Phase 0.6's `reap_stale_transfer_temps` already lists `dest_path` once, immediately before the spawn loop, on every copy. So the driver ALREADY pays for one destination listing whose result would answer the general case — including merges, the common case above. That makes the COST side of the "list once up front" option zero. It does NOT answer what killed that option: one up-front listing goes stale across a multi-minute batch, so a file arriving mid-batch would be missed and an Overwrite would replace it with no prompt. David re-decided with the cost fact on the table (2026-08-02) and kept the conservative fix. ❌ Recorded as a fact, not a proposal.
...
**Decision**: the window was NOT the throughput story; the per-file destination pre-check was, and it is now skipped for a destination directory the operation created (see "Skipping the destination pre-check" above).
