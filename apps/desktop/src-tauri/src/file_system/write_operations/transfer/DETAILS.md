...
**Not taken, and worth knowing about.** Phase 0.6's `reap_stale_transfer_temps` already lists `dest_path` once, immediately before the spawn loop, on every copy. So the driver ALREADY pays for one destination listing whose result would answer the general case — including merges. Turning that listing into the conflict answer is the "list once up front" option above; it is strictly more valuable and strictly less safe, and it needs its own decision rather than being smuggled in as an optimization.
