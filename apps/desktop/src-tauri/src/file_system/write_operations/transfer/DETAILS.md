...
**Decision**: the concurrency window stays `min(src.max_concurrent_ops(), dst.max_concurrent_ops(), 32)`, and the per-file destination pre-check is the thing worth attacking instead.
**Why**: measured, not argued: `volume_copy_concurrency_bench.rs` sweeps the window 1-32 on unchanged code against a real QNAP over direct smb2 (~3.7 ms RTT) and against Docker Samba. On the NAS the curve is flat from window 12 up (12/16/24/32 medians 3.302 / 3.245 / 3.278 / 3.224 s for 500 x 16 KiB, spreads overlapping), and 8 -> 16 buys 1.16x. The few-large shape is link-bound (~97 MB/s, saturated gigabit) and shows no measurable difference at any width from 4 up. What the flat part IS: the concurrent spawn loop's `dest_volume.get_metadata(&dest_item_path)` runs once per top-level source ON THE DRIVER before the task is spawned, so a batch carries a hard floor of `N x RTT` that no window width can overlap. Measured directly (`serial_precheck_floor`, outside the driver): 2.378 s of a 3.224 s best run, **74%**. ❌ Don't reach for the window formula to make transfers faster; the ceiling is the serial probe, and the fix shape is the one the deep-merge path already uses (one dest listing per level, `name -> FileEntry` map, no per-child probes). Full tables, both targets, both shapes, what the local-side CPU heuristic costs the `network.smbConcurrency` setting, and a scoped (unimplemented) proposal for the pre-check with its staleness and MTP constraints: `docs/notes/transfer-concurrency-window-bench-2026-08-02.md`.
