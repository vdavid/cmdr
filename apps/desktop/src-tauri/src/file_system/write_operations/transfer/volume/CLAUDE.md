...
- **The merge invariant**: a merge never deletes or overwrites a dest file the source doesn't shadow, under every
  policy, backend, and cancel/rollback/retry mid-merge. Assert it through `safety_oracle.rs`, ❌ never inline;
  new cells go in `safety_grid_tests.rs`.
- **Dir-vs-dir is NEVER a conflict**, only file-vs-file. **Overwrite means merge for dirs, replace for files**, enforced
