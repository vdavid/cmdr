...
- Pinned by `merge_case_fold_tests.rs` (case, normalization, a case-SENSITIVE destination keeping both spellings with no prompt, an unanswerable probe, and a folded directory child merging into the directory that is there). The same-volume engine's twin is `rename_merge.rs::late_detected_collision`.
...
**A blanket Overwrite never crosses types.** `Overwrite`, `Overwrite all smaller`, and `Overwrite all older` are answers about two FILES: "the one at the destination is stale, put the source there". Across types the act is a different thing — replacing a folder with a file throws a tree away, replacing a file with a folder throws the file away — and nobody picking a policy in the transfer dialog was shown either. So `resolve_volume_conflict` turns any of the three into `Skip` when `source_is_directory != destination_is_directory`, whether it came from `config.conflict_resolution` or from an apply-to-all carry (both the up-front lookup and the Stop arm's under-the-lock re-check). The item reports as skipped, and a MOVE's source sweep spares it like any other skip. The rule itself is `../../conflict.rs::blanket_resolution_across_types`, shared with the local-FS and in-archive engines so all three answer the same. The conditional variants couldn't even ask their own question across types: a directory's reported size is its own entry's, so any ordinary file looked "bigger" and "Overwrite all smaller" cleared destination folders wholesale. An Overwrite a person picked on a Stop prompt for THAT pair still replaces — the dialog names both types, and the click is the consent a blanket policy lacks. Pinned by `conflict.rs::{a_blanket_overwrite_never_clears_a_folder_a_file_landed_on, a_blanket_overwrite_never_deletes_a_file_a_folder_landed_on, a_latched_overwrite_all_never_clears_a_folder_a_file_landed_on}`.
