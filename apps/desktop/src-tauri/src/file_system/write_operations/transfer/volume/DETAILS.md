...
- **`conflict.rs`'s tests are four `#[path]` children, one per question the resolver answers**: `conflict_tests.rs`
  (merge safety, the overwrite ordering, the finalize swap), `conflict_conditional_tests.rs` (`OverwriteSmaller` /
  `OverwriteOlder`, on hints and on `get_metadata`), `conflict_same_item_tests.rs` (`is_the_same_volume_path`), and
  `conflict_cross_type_tests.rs` (a blanket policy never crosses types). They're declared at the bottom of
  `conflict.rs`, so a test name reads `conflict::<child>::<test>` rather than the flat `conflict::tests::`.
