...
**Decision**: Both the local-FS walker (`walk_dir_recursive`) and the volume-aware delete walker (`scan_volume_recursive`) dedupe hardlinks by inode when summing `total_bytes`, and the per-entry `progress_bytes` field carries the same dedup into the active phase.
**Why**: A naïve `*total_bytes += metadata.len()` per direntry over-counts on hardlink-heavy trees (cargo `target/`, sccache caches, deduplicated backups). Without dedup, a 49 GB `target/debug` reported 70+ GB to the scan UI, and the "X% of estimated" progress bar (denominator from the indexer's `dir_stats`, which already inode-dedupes) couldn't converge to 100%. Mirrors `indexing/scanner.rs`'s `seen_inodes: HashSet<u64>` pattern, with the same `nlink == 1` fast path. The set is operation-scoped (shared across all source roots in one scan, dropped when the scan ends), so hardlinks crossing source roots still count once. **Unix-only**: `std::fs::Metadata` has no `nlink()` accessor outside Unix; non-Unix falls back to the old naïve sum. Doesn't apply to `dry_run_scan_recursive` (that path reports for conflict counts, not for a progress denominator). Active-phase counters (delete walker, `copy_single_item`, volume delete walker) sum the per-entry `progress_bytes` (`= size` for first-seen inode, `0` for hardlink dupes) so `bytes_done` stays aligned with `total_bytes` — without this, a hardlink-heavy real-world repro overshot 81.6 GB / 59.84 GB ≈ 136% on the bar. For copy, the chunked-progress callback scales mid-file chunk bytes by `progress_weight / metadata.len()` so dupes don't briefly inflate the bar during streaming. The volume-aware variant uses `FileEntry::inode` (populated by `LocalPosixVolume` for files with `nlink > 1`, left `None` by MTP/SMB/InMemory) and a `seen_inodes: HashSet<u64>` carried through `scan_volume_recursive`'s recursion — covers cargo `target/` on external SSDs and Time Machine HFS+ backups routed via `path_to_id("/Volumes/...") ≠ "root"`. Pinned by `delete/hardlink_progress_tests.rs`, `transfer/hardlink_progress_tests.rs`, and `delete/volume_hardlink_progress_tests.rs`. **Volume copy** (`copy_volumes_with_progress`) is the one remaining un-dedup'd path — see the gotcha below; status quo is bar fills correctly but absolute byte label is inflated.
...
**Gotcha**: Volume copy doesn't dedupe hardlinks in progress accounting (`local_path` ≠ `"root"`).
**Why**: `delete_files_with_progress` (local-FS path) and `delete_volume_files_with_progress` (volume path, including `LocalPosixVolume` for external SSDs and `/Volumes/...`) both dedupe hardlinks now via `FileInfo::progress_bytes` / `VolumeDeleteEntry::progress_bytes`. Volume copy through `copy_volumes_with_progress` does NOT — its per-file byte credits flow through chunked-stream `on_file_progress` callbacks built by `transfer_driver::make_concurrent_per_file_progress`, with no per-file inode tracking. The orchestrator also has a post-task "credit `last_file_bytes` if the backend never invoked on_progress" compensation in the `FuturesUnordered` branch. Threading inode-dedup through both paths is doable but non-trivial. Status quo: scan-side `total_bytes` and the per-task byte credits are both un-dedup'd, so the bar fills 0→100% smoothly and never overshoots — only the absolute "Copying N GB" label is inflated by hardlink count on `target/`-on-external-SSD-shaped sources. Real but cosmetic. Repro shape lives in `delete/volume_hardlink_progress_tests.rs` (works because the delete pipeline is fixed); a copy-side equivalent test exists in git history if the follow-up needs it. When fixing, plumb `seen_inodes` through `volume_strategy::copy_directory_streaming` AND wire `FileEntry::inode` into a per-file "counts toward progress" flag the per-task closure can consult.
