...
- Top level: `mod.rs` (public API + `start_write_operation` spawn lifecycle), `types.rs` (serializable events/config +
  `OperationEventSink`), `state.rs` (the two caches, `WriteOperationState`, `CopyTransaction`, settle guard),
  `validation.rs`, `conflict.rs`, `overwrite.rs`, `durability.rs`, `cancellable.rs`, `scan.rs`, `scan_preview.rs`,
  `eta.rs`.
