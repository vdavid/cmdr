...
**Why**: `walk_dir_recursive` dedupes hardlinks by inode for `total_bytes` AND populates `FileInfo::progress_bytes` so the active phase agrees (so a hardlink-heavy tree like cargo's `target/` reports correct bytes-to-free and the active-phase bar doesn't overshoot). `FileEntry` doesn't carry inode, so when the oracle supplies one half of a hardlink pair from the cached listing (via `walk_cached_entries`) and a real walk supplies the other half, the dedup misses on the cached side: both `total_bytes` and the cached entry's `progress_bytes` count the dupe as a unique inode. Direction is safe: over-counting denominator + matching numerator → bar still ends at 100%, ETA stays pessimistic, disk-space check stays conservative. Only triggers when a watcher-fresh sub-listing for the subtree exists in a sibling pane — not the common case (the parent pane's listing is for the source's parent, not the subtree itself). If true cross-boundary dedup ever becomes worth it, add `inode: Option<u64>` to `FileEntry` and consult it in `walk_cached_entries`.
