...
| `transfer_driver.rs` | Shared per-source transfer driver for copy/move ops. Owns the bulk-skip prelude, per-iter cancellation check, conflict-resolve dispatch (async path), per-iter skip accounting, and paired progress/status emit. Two sibling entry points: `drive_transfer_serial_sync` (for local copy/move; closure captures `&mut CopyTransaction` etc.) and `drive_transfer_serial_async` (for volume ops; driver owns top-level conflict detection + dispatch). Lands ahead of callers in M1.5; production callers migrate in M2 step 8 / M3 (per `docs/specs/transfer-driver-refactor-plan.md`). |
| `tests.rs` | Unit tests. |
