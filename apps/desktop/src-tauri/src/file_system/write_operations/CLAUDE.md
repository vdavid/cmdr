...
- Spine: `manager.rs` (registry, lanes, admission), `state.rs` (op state, `CopyTransaction`, cancel/abort),
  ...
  `source_binding.rs`. Own docs: `transfer/`, `delete/`, `archive_edit/`, plus
...
- **Emit through `OperationEventSink`, ❌ never `AppHandle`**, built at the IPC edge. `write-settled` fires once,
...
- **A caller may BIND the sources an op may touch** (`source_binding.rs`, an `Option` on every starter): one no
  longer matching its promised identity is dropped before any I/O and reported `Skipped`. Every volume route carries
  it, and the EXPECTATION picks the namespace: capture locally when the source volume has a `local_path()`. ❌ Nothing
  here asks WHO started an op (`approved_op_parity_tests.rs`).
...
- **Register a destination with `downloads::note_pending_write_for_cmdr` BEFORE the syscall** (renames register
  both halves).
...
- **Scans report `total_bytes` (copy/move) and `dedup_bytes` (delete)**: ❌ never point copy at the dedup'd one, it
  under-reserves disk.
- **Every managed mutation journals by `op_id`**, and a VOLUME op passes its REAL volume id
...
- **`LifecycleStatus` is the ONE lifecycle answer**, carried as `OperationStatus.lifecycle`. ❌ Never re-derive it
