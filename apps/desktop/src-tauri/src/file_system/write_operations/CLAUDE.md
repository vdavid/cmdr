...
**`write-error` carries a provider-enriched `FriendlyError` from the move path, a variant-derived one from the copy path.** `move_between_volumes` keeps the originating `VolumeError + path` alongside each `?`-propagated `WriteOperationError` (via the `MoveOpFailure = (WriteOperationError, Option<(VolumeError, PathBuf)>)` tuple), so the outer emit can call `WriteErrorEvent::with_friendly` and run the full `friendly_error_from_volume_error` + `enrich_with_provider` pipeline — matching the listing-error path's quality. `copy_between_volumes` doesn't thread the volume context through `copy_volumes_with_progress` (its return type is consumed by tests via `CollectorEventSink`, and the function is 600 lines), so copy emits via plain `WriteErrorEvent::new` which derives the friendly from the `WriteOperationError` variant via `friendly_from_write_error`. The user-visible difference: provider-specific suggestions ("This folder is managed by **MacDroid**…") fire on move failures, while copy failures get the generic friendly copy. Promoting copy to the same level is feasible but its own piece of work.
