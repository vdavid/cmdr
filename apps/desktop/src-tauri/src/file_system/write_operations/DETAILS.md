...
**A blanket Overwrite never crosses types.** `Overwrite`, `OverwriteSmaller`, and `OverwriteOlder` all say "the entry at the destination is stale, put the source there", which is an answer about two FILES. Across types the act is a different one — replacing a folder with a file discards the folder, replacing a file with a folder discards the file — and nobody picking a policy in the transfer dialog was shown either. So `conflict::blanket_resolution_across_types` reduces any of the three to `Skip` on a cross-type clash, and the item reports as an ordinary skip. It governs the resolution nobody looked at per item: the configured `conflict_resolution` AND an apply-to-all carry (which is why the two-bucket latch's own refusal to carry an Overwrite into a file→folder clash isn't enough — a config policy that IS `Overwrite` walked straight past it). All three engines call it: local (`conflict.rs`), cross-volume (`transfer/volume/conflict.rs`, both the up-front lookup and the Stop arm's under-the-lock re-check), and in-archive (`archive_edit/conflicts.rs`, both `ConflictMode` arms). An Overwrite a person picked on the Stop prompt for THAT pair still replaces: the dialog names both types (`build_conflict_event`'s `source_is_directory` / `destination_is_directory`), and that click is the consent a blanket policy lacks. The conditional variants couldn't ask their own question across types anyway: a directory's `len()` is its own inode's, so every ordinary file looked "bigger" and "Overwrite all smaller" deleted destination folders wholesale.
