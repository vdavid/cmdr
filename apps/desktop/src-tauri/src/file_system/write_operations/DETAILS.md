...
- **`RoutedKind` rather than a bool, because the two routes part ways on a MOVE.** An archive can delete its source entries after the extract lands (the compound move-out op), a snapshot cannot: `.git/branches/main/README.md` has no file to remove. So `start_volume_move` refuses a `GitPortal` source with the typed `ReadOnlyDevice`, named for the source volume, before the destination is even resolved. A `GitPortal` DESTINATION is refused the same way in both `start_volume_copy` and `start_volume_move`: every mutation on that volume answers `NotSupported`, so a drop onto a snapshot would otherwise start and die on its first write. A `.zip` destination is NOT this case; it routes to the archive-edit driver, which really does write.
