...
- **Concurrent copy path caveat.** `copy_volumes_with_progress`'s `FuturesUnordered` concurrent path has no single between-files boundary, so v1 does NOT honor mid-batch pause there (its per-file callback is cancel-only). Documented + pinned: see `transfer/DETAILS.md` § "Pause and the concurrent copy path" and `transfer_driver::tests::concurrent_per_file_callback_is_cancel_only_not_pause_aware`. Serial paths (local copy/move, cross-volume serial, delete) honor pause between files.
