...
Backend directory reading, caching, sorting, and streaming for the file explorer: 100k+ entries, non-blocking I/O,
progress events.
...
- **Refreshes of ONE directory stay serialized** (`notify_full_refresh`): concurrently an older read lands last,
  stranding a pane. `DETAILS.md` § "Serializing full refreshes".
...
- **`read_directory_with_progress` holds a `priority::foreground` lease for its whole body**, so an SMB upload and the
  index scan stand aside. RAII: ❌ never bind it to `_`. `DETAILS.md` § "The foreground lease".
...
Data flow, the caching lifecycle, the orphan reaper, decisions, helper catalogs, diff coalescing, metadata tiers, the
overlay step, and tags: `DETAILS.md`. Read it before any non-trivial work here.
