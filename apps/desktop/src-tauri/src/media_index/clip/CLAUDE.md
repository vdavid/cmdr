...
- **ONE dedicated 8 MB-stack `clip-worker` thread owns both towers and serializes every predict.** `MLModel` is `!Send`
  and a synchronous ANE predict is an XPC round-trip that can overrun a small stack; only input ids / pixel `Vec`s go
  in and `Vec<f32>` embeddings come out. ❌ Never touch an `MLModel` off that thread.
...
The model evidence, the towers, install + reclaim, the gate, and the query path: `DETAILS.md`. Read it
