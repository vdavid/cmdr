...
The M1.5b UI (per-volume SMB opt-in + "always index" toggles + i18n + E2E) binds to the backend `media_index_set_*`
commands + `media_index_volume_state`'s network fields this slice left (see [DETAILS.md](DETAILS.md)). Full MTP on-demand
wiring beyond the never-background gate; the importance slider + progress counts/ETA (M2); tags, embeddings, faces (M2+).
