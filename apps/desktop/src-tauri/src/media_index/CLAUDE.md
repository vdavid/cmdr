...
Image-ML enrichment: images searchable by content. Read-consumer of `indexing/`, sibling to `importance/`. Off by
default. On-device OCR + Vision tags + similarity embeddings; local enriches by default, opt-in SMB conservative, MTP
never. Real macOS Vision, a fake for tests. Plan:
[`docs/specs/media-ml-index-plan.md`](../../../../../docs/specs/media-ml-index-plan.md).
...
- **Disposable, path-keyed cache.** A schema bump/corruption delete-and-recreates `media.db` (`SCHEMA_VERSION` 2, no
  ...
  decode → OCR + tags + feature print); `engine_version` folds the revisions and drives staleness.
- **GC is deletion-driven + edge-triggered (data-safety).** GC ONLY on a `Completed` bus edge (`borrow_and_update`,
  ...
  vanished files collect. Never persist the lifecycle-bus `generation` (a transient wake counter).
- **Importance-prioritized (headline).** Filter + order by `ImportanceIndex` at the slider threshold. `folder_scores`
  `None` (unscored) ⇒ DEFER to override-only, NEVER enrich-all (forward-only ⇒ a first-run race would over-index
...
Still open: per-FOLDER exclude + "always index" triggers (setters ready), MTP on-demand, later (CLIP, faces, captions).
