...
- **`SCHEMA_VERSION` is a cache version, not a migration target.** Bumping it delete-and-recreates every user's
  ...
  migration.
- **`media_ocr` is a STANDALONE fts5 table, not external-content.** External content would sync via triggers off
  another table's integer rowid; the standalone table keyed by an UNINDEXED `file_id` keeps enrichment and GC a plain
...
- **`needs_enrichment` = `(path, mtime, size)` + the analyze stamp.** State is DELIBERATELY excluded, so a failed file
  isn't re-hammered every completed scan; a real file change re-tries it. CLIP staleness is a separate key
  (`needs_clip` over `clip_stamp`), decoupled on purpose.
...
The table-by-table schema, the f16 + integer-id decisions, and the read-path audit: `DETAILS.md`. Read it
