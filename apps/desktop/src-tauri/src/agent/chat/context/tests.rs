...
fn a_hundred_file_rename_turn_needs_more_than_the_default_budget() {
    use crate::agent::chat::budget::{MAX_TOOL_RESULT_TOKENS, PROMPT_BUDGET_60K};

    const FILES: usize = 100;
    /// Average OCR text per screenshot in the real corpus this came from, well under
    /// `image_facts`' 2,000-char per-file cap.
    const OCR_CHARS: usize = 900;

    let listing = json!({
        "pane": "right",
        "path": "/Users/me/Downloads/shots",
        "scope": "selection",
        "returned": FILES,
        "total": FILES,
        "truncated": false,
        "entries": (0..FILES)
            .map(|i| json!({ "name": format!("2026-07-21 CleanShot{i:06}@2x.png"), "size": 4_302_190, "modified": 1_785_247_668 }))
            .collect::<Vec<_>>(),
    });
    ...
    let facts_row = |i: usize| {
        json!({
            "path": format!("/Users/me/Downloads/shots/2026-07-21 CleanShot{i:06}@2x.png"),
            "state": "indexed",
    ...
    for (page, indexes) in pages.iter().enumerate() {
        ...
        transcript.push(assistant_tool_call(
            &call_id,
            ToolId::ImageFacts,
            json!({ "volumeId": "root", "paths": indexes.iter().map(|i| format!("/Users/me/Downloads/shots/2026-07-21 CleanShot{i:06}@2x.png")).collect::<Vec<_>>() }),
            1_030 + page as i64,
        ));
    ...
    transcript.push(assistant_tool_call(
        ...
        json!({ "renames": (0..FILES).map(|i| json!({
            "sourcePath": format!("/Users/me/Downloads/shots/2026-07-21 CleanShot{i:06}@2x.png"),
            "volumeId": "root",
            "destinationName": format!("2026-07-21 19-36-00 cmdr-ai-rename-review-dialog-{i}.png"),
