...
- what the PROMPT holds: `(budget − 10% headroom − 5,319 of prefix) / 349 per file`. The headroom exists because the
...
- **Fixed overhead: 5,319 tokens** on every single call — 1,636 for `SYSTEM_PROMPT` and 3,683 for the 18 tool
...
- **A 100-file content-based rename: 41,901 tokens** for the whole turn. The parts above account for over 90% of it; the
