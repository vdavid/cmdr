...
- what the PROMPT holds: `(budget − 10% headroom − 5,241 of prefix) / 349 per file`. The headroom exists because the
...
- **Fixed overhead: 5,241 tokens** on every single call — 1,636 for `SYSTEM_PROMPT` and 3,605 for the 18 tool
...
- **A 100-file content-based rename: 41,823 tokens** for the whole turn. The parts above account for over 90% of it; the
