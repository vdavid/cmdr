...
messages (typed `content_blocks` JSON), an FTS5 search index over message text, a per-day cost meter, and the proposal
spine. Depth (DDL rationale, the FTS design, the search-JOIN gotcha, no-retention-in-v1): `DETAILS.md`.
