...
- The pure core: `coalesce.rs` (counters), `interest.rs` (score, tier, delay), `compact.rs` (the budgeted digest),
  ...
  halves: `prepare_wake`, `run_prepared_wake`). `persist.rs` is the one file here taking a `Connection`.
- The driver: `channel.rs` (the tap's lane), `writer.rs` (the thread owning the `Inbox`, its write connection, the
  timer), `runner.rs` (the wake thread, its outcome line), `snapshot.rs` (cached readiness), `indicator.rs` +
  `staged.rs` (the corner's event, the toast's), `importance.rs` (TTL-cached weights), `settings.rs` (cadence,
...
- **Consent outranks disk access outranks the key**, cached in `snapshot.rs`. Without consent the pipeline stores
  nothing and **purges what it stored** (`purge_if_not_permitted`, at launch and on readiness change; a copy bump
  un-accepts everybody). No key: signal accumulates. The corner renders the last two gaps, silent on `NeedsConsent`
  and on `proactive` off.
