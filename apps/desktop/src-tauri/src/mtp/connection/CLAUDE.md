...
- **Foreground-priority scheduler (`scheduler.rs`)**: the shared USB pipe has two access classes. ❌ Every foreground
  device op (nav, copy, delete, rename, move, upload, visible-pane resolve) MUST take `foreground_guard(device_id)` for
  its lifetime, or the scan won't yield to it (the livelock returns). The scan lists via `list_directory_for_scan` (never
  `list_directory*`): per-unit acquire + `background_yield_point` between units. ❌ Gate the live index feed BEFORE any
  device resolve — `feed_index_added_or_changed` calls `indexing::buffer_mtp_handle_if_scanning` first (buffers the RAW
  handle during a scan, replayed after). Deadlock-freedom + sizing: [DETAILS.md](DETAILS.md) § "Foreground-priority device scheduler".
- **`resolve_path_to_handle()` is cache-only**: returns `ObjectNotFound` if the path hasn't appeared in a prior
  ...
  the USB call. (Its inverse `resolve_handle_to_path()` *does* walk on demand — for pathless PTP events, not browsing.)
...
- **Disconnect from the event loop must clear the device registry**: on `next_event()` → `Error::Disconnected`,
  ...
  storage Stale (freshness D4) — don't drop it, or a Fresh index lies after an unplug.
- **The event loop feeds the per-volume index, not just the live pane.** `ObjectAdded`/`ObjectInfoChanged` →
  ...
  the STORED handle via `find_entry_by_inode`). Buffering: `indexing/mtp_watch.rs`; details in
  [DETAILS.md](DETAILS.md).
- **`MtpDisconnectReason` is load-bearing for logs/UI**: pass `User` only for the settings-toggle / explicit-disconnect
  path; hotplug loss and I/O drops are `Removed`. Misclassifying makes unstable USB read as repeated unplugs.
...
- **Stale cached parent handle on upload self-heals, then signals a one-shot retry.** A re-keyed handle makes
  ...
  per-device `Mutex` isn't reentrant → deadlock), and never map this to a hard not-found. Mechanism:
