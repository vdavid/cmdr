...
- **❌ Never wrap an mtp-rs call in `tokio::time::timeout`, and never abort a task holding one.** The deadline DROPS
  ...
  [DETAILS.md](DETAILS.md) § "No dropping timeouts".
...
- **❌ A `SessionReset` (mtp-rs `DeviceReset`) is NOT a disconnect** — only the PTP session died. `session_reset.rs`
  ...
  (`pnpm check mtp-no-transport-reset`). Failing ops report the RETRYABLE `DeviceSessionReset`. [DETAILS.md](DETAILS.md) §§ "Session reset is not a disconnect", "No transport reset in
  recovery".
