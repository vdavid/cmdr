...
picker, reactive volume state). The frontend is a passive consumer of `volumes-changed` and `mtp-device-connected` /
...
- **Write-capability probe.** `probe_write_capability()` creates a hidden `.cmdr_write_probe` folder to catch cameras
  that advertise write support but reject writes (`StoreReadOnly`). Timeouts and non-fatal errors count as writable.
...
- **❌ The session layer never registers volumes.** `connect()` attaches storages through the `OnceLock` registrar in
  `connection/volume_registrar.rs`, installed at startup by `MtpVolume::install_volume_registrar`. Keep it synchronous:
  the attach must finish before the event loop starts. New backends copy this; `DETAILS.md`.
