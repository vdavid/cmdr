...
Auth-flow design background: `docs/notes/smb-auth-flow-redesign.md`.
...
- **Credentials never go into argv.** `smbclient` gets creds via a 0o600 `-A <file>` (never `-U user%pass`); `gio mount`
  via child stdin (never `sh -c "echo PASS | …"`); `build_smbutil_url` only builds passwordless `//host` URLs. `ps aux` /
  `/proc/<pid>/cmdline` must never see a password. Don't reintroduce a URL-embedded or argv-passed password on any path.
- **Compare servers by identity, never by string.** Use `server_identity::same_server` / `same_server_live` /
  ...
  splits one NAS in two, breaks session reuse, forces a duplicate mount, and (for creds) saves under one key but looks
...
- **mDNS is gated, not unconditional.** Startup fires mDNS only if `network.enabled && (firstTriggerDone || smb-e2e)`.
  ...
  user takes a network action (which calls `ensure_network_discovery_started` and flips the flag). Runtime mirror:
  `network::NETWORK_ENABLED` (`AtomicBool`); BE upgrade paths check `is_network_enabled()` before mDNS.
- **Every NetFS mount sets `UIOption = NoUI`** (`open_option_entries` in `mount.rs`). Without it, NetFS hands auth
  ...
  `kNetAuthErrorInternal` (-6600) on dismiss. `NoUI` makes failures return as typed codes for the frontend's own login
  form.
- **`register_replacing_predecessor` (in `smb_upgrade.rs`) must run on every re-register**, not a bare overwrite. It
  looks up the predecessor, calls `on_unmount` on it (cancels the old watcher, drops the old smb2 session), registers
  the new volume, then emits `volumes-changed`. A bare overwrite leaves the old watcher's lifetime non-deterministic and
  lets an in-flight reconnect install a session into a volume no longer in the manager. It wraps `on_unmount` in
  `spawn_blocking` (it uses `blocking_*` locks; a direct call panics with "cannot block_on within a runtime"). Don't
  revert to a direct call.
- **Strip `.local` from the addr for smb2** (`build_addr` in `smb_connection.rs`): `smb2` puts `server_name` into UNC
  paths and some servers reject `foo.local` in `\\foo.local\IPC$`. Pass the resolved IP from mDNS when available.
- **All three SMB-upgrade paths share resolution.** Startup (`file_system::upgrade_existing_smb_mounts`) and mount-time
  ...
  `resolve_ip_to_hostname_with_wait` + `get_keychain_password` pair. Don't let the resolver choice drift: the one-shot
  resolver misses hostname-keyed creds and falls back to guest → `STATUS_LOGON_FAILURE`.
...
- **Pass the port as a separate parameter, never embedded in the server string** (embedding makes `build_smb_addr` double
  it, `localhost:10480:10480`).
