...
- **mod.rs** -- Public API (`init()`, `start_indexing()`, `stop_indexing()`, `clear_index()`), `IndexPhase` state machine, the `INDEXING` static mutex. Thin API functions that lock `INDEXING` and delegate to `IndexManager`. Integration tests.
