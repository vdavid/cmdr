...
- **The `INDEX_REGISTRY` (`Mutex<HashMap<VolumeId, IndexInstance>>`) is the authority** (absent key = disabled, no
  ...
  under it. Enrichment SKIPS when `get_read_pool_for` is `None`.
...
- **`start_indexing` is lock-first**: reserve the registry slot atomically BEFORE building `IndexManager`, else two
  starts for one volume race writer threads on its DB. **Never hold `INDEX_REGISTRY` across a blocking or
  registry-re-entrant manager call** (froze the UI on real hardware): `stop_indexing` / `clear_index` drop the guard
  before `shutdown()`; `force_scan` and the journal-gap fallback drop it before `start_scan`. DETAILS § "Drop the
  registry guard".
- **Reconciler/event loops hold a READ connection, never a write one** (write-mode pragmas can `SQLITE_BUSY` and silently
  ...
  (uncommitted inserts → double-assign).
- **Schema invariants.** Register the `platform_case` collation on every connection (not persisted; else the raw
  ...
  a multi-TB ghost-size hazard) nor `name_folded`. The scanner uses `INSERT OR IGNORE`, never `INSERT OR REPLACE`
  (reassigns IDs, orphans children, slow on a populated DB).
...
- **Defer `root` auto-start until FDA is decided** (`should_auto_start_indexing`): a scan from `/` stacks TCC popups over
  the FDA modal. FDA gates ONLY `root` — don't route SMB/MTP through it.
...
- **Freshness has ONE transition table (`freshness.rs`); don't branch elsewhere.** No journal ⇒ a persisted index
  ...
  lock rule above), not registry-locking `apply_freshness_event`.
...
- **Threads + resources.** Memory watchdog: one GLOBAL 16 GB budget (`stop_all_indexing`), not per-volume; start is
  ...
  macOS (else multi-GB leaks). Spawn with `tauri::async_runtime::spawn`; `tokio::spawn` panics from the sync `setup()`
  hook (where indexing can start).
