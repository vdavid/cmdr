...
[`store/`](store/CLAUDE.md). Top-level: `state.rs` + `manager.rs` (lifecycle), `local_reconcile.rs` /
`volume_scanner.rs` (LOCAL / SMB-MTP scan), `reconciler.rs` + `event_loop.rs` (live), `enrichment.rs`, `freshness.rs`.
IPC `commands/indexing.rs`, FE `src/lib/indexing/`, search `src/search/`.
...
- **`INDEX_REGISTRY` (`Mutex<HashMap<VolumeId, IndexInstance>>`) is the authority**: absent key = disabled. Guards
  lifecycle ONLY; reads route through the per-volume `ReadPool`.
...
- **The watcher→loop channel is UNBOUNDED**: backpressure dropped FSEvents and forced full scans. Don't re-bound it;
  `classify_ingestion_pressure` caps memory instead.
...
- **Structural rewrites repair ancestors ON THE WRITER**; full-aggregate senders declare `source: Maps|Sql` (`Maps` only
  for a fresh scan).

Coverage epochs and verification cost:

- **Never write `listed_epoch = 0` for a directory we DID list but chose to skip.** `0` absorbs up the whole chain, so
  one skipped dir renders `~` incomplete. Post-replay verification declines oversized dirs and leaves the epoch alone
  (DETAILS § "Bounding verification cost").
- **`verify_affected_dirs` guards BOTH phases**: a `LIMIT`-probe before the DB snapshot, and a `read_dir` ITERATION cap
  (not an upsert cap — the loop skips known children before doing work).
...
- **Gated on a `direct` (smb2) connection; an `os_mount` upgrades first** (typed `SmbIndexGateReason`). Reconnect
  ...
  (`force_scan`): SMB/MTP → `start_volume_scan`, local → `start_scan`.
...
Flows, decisions, and gotchas: [DETAILS.md](DETAILS.md). Read it before any non-trivial work here.
