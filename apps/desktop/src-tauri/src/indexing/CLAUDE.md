...
- **Schema v6**: Added `name_folded` column storing pre-computed `normalize_for_comparison(name)` (initially macOS only, unified to all platforms in v9). Replaced `idx_parent` with `idx_parent_name_folded(parent_id, name_folded)`. `resolve_component` now queries the index directly instead of fetching all children and matching in Rust.
...
**Writer-centralized delta propagation**: `UpsertEntryV2`, `DeleteEntryById`, and `DeleteSubtreeById` all auto-propagate `dir_stats` deltas in the writer thread. Callers only need to send standalone `PropagateDeltaById` for post-`scan_subtree` propagation (subtree totals that the scan computed via `ComputeSubtreeAggregates`). This prevents bugs where a caller forgets to propagate (for example, the reconciler previously didn't propagate size deltas for `item_modified` events, leaving parent sizes stale until the verifier caught it on navigation).
...
**`reconcile_subtree` auto-creates new root directories**: When `must_scan_sub_dirs` fires for a newly created/copied directory (not yet in DB), `reconcile_subtree` resolves the parent, upserts the root directory via `UpsertEntryV2` + `flush_blocking()`, then proceeds with the diff. Previously it returned early with `added: 0`, leaving the entire subtree unindexed.
