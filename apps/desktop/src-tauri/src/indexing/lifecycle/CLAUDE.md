...
- **`INDEX_REGISTRY` is the authority** for which volumes are indexed. It guards lifecycle ONLY; reads route through
  the per-volume `ReadPool`, never under the lock. **Disabled = the ABSENCE of a key** (no `IndexPhase::Disabled`);
