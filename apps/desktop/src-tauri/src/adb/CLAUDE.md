...
commands. The wire and the `Volume` are `crates/cmdr-adb/` (read its `CLAUDE.md`). Same split as `mtp/volume_wiring.rs`
and `network/sftp_volume_wiring.rs`: the wiring knows the backend and the registry, neither knows the wiring.

## Module map

- `device_provider.rs`: `AdbDevices` (the tracker's last list plus connected volumes by serial, one process-wide
  `RwLock`), `readiness_of` (which device states become a row), and `AdbDeviceProvider`.
- `volume_wiring.rs`: `install_device_provider`, `start_adb_tracker`, `set_adb_settings`, `connect_adb_device`,
  `cancel_connect`, `volume_id_for_path` (what `commands/volumes.rs::resolve_path_to_volume` calls).
...
- **❗ A device is listed before it's connected, and before it's authorized.** `entries()` carries a
  `device_readiness`; only `recovery` / `bootloader` / `sideload` / an unreadable state word are left out. ❌ Never
  dial from `entries()`: the listing runs on every `volumes-changed`.
...
- **❗ Turning `fileOperations.adbEnabled` off empties the cached list, not only the tracker**, or the last list stays
  frozen on screen with its volumes registered. The two settings travel together through `set_adb_settings`.
- **❗ No `adb` binary STOPS the tracker** rather than retrying, and `recheck_adb_install` is the only way back and the
  only path allowed to retry `adb start-server`. ❌ Nothing may poll it: one attempt per human action.
...
Flows, the provider's answers, eject, the settings, and what's not wired yet: `DETAILS.md`.
