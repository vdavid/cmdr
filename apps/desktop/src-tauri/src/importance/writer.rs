    ...
    while let Ok(msg) = receiver.recv() {
        match msg {
            WriteMessage::WriteWeights { generation, rows } => {
                if let Err(e) = apply_weights(&mut conn, generation, &rows) {
...
/// Apply a recompute pass under one transaction: upsert every weight row at
/// `generation`, then advance the stored generation. Doing both in one
/// transaction keeps the generation and the rows consistent — a reader never sees
/// a bumped generation with un-written rows, or vice versa.
fn apply_weights(conn: &mut Connection, generation: u64, rows: &[WeightRow]) -> Result<(), ImportanceStoreError> {
