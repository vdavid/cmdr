...
- **A missing optional signal REDISTRIBUTES, never fabricates.** `SignalSet` says which optional signals a volume's
  ...
  substitute a default value for an unavailable signal, and ❌ don't conflate *unavailable* with an unsampled `None`
...
- **`SignalKind::ALL` drives every loop.** A new signal needs the `ALL` array plus arms in `raw_signal_value`,
  `Weights::additive_weight`, and `EffectiveWeights::weight_of` — miss one and the signal silently weighs nothing.
  Full recipe: `DETAILS.md` § Adding a signal.
...
- **`FolderSignals`'s serde shape is load-bearing**: `camelCase` + `specta::Type` + per-field `skip_serializing_if`
  (the trimmed JSON the store persists) + `#[serde(default)]` (any subset deserializes, so a row written before a field
