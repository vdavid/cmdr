...
| `download-llama-server.go` | Build-time downloader for the llama-server binary. Invoked from `src-tauri/build.rs`. In linked git worktrees, symlinks from the main clone when the `.version` matches. In CI release builds (`APPLE_SIGNING_IDENTITY` set) it codesigns each extracted binary; when `LLAMA_SIGN_KEYCHAIN` is set it targets that keychain explicitly via `codesign --keychain` (release.yml sets one up because the login keychain's key isn't usable from the launchd-launched runner) |
