 ...
 */
export function sanitizeWorktreeSlug(raw: unknown): string | null {
...
export function resolveInstanceId({
  ...
  isDev: boolean
  /** the current value of CMDR_INSTANCE_ID, if any */
  envInstanceId: string | undefined
  /** raw --worktree argument (pre-sanitization) */
  ...
  if (worktreeSlug != null) {
    ...
    if (slug === null) {
      throw new Error(
        `--worktree must be 1-${String(MAX_SLUG_LEN)} alphanumeric or dash characters after sanitization (got: ${JSON.stringify(worktreeSlug)})`,
      )
    }
    return `dev-${slug}`
  }
 ...
 * file replaces in tauri-wrapper.ts.
 */
export function computeAppDataDir({
  identifier,
  platform,
  home,
  xdgDataHome,
 ...
 */
export function productName(instanceId: string | null): string {
 ...
 */
export function extractWorktreeFlag(argv: string[]): { slug: string | null; rest: string[] } {
 ...
 *
 * @param name e.g. "tauri-mcp.port"
 */
export function writePortFile(dir: string, name: string, port: number): void {
 ...
 */
export function removePortFile(dir: string, name: string): void {
  try {
