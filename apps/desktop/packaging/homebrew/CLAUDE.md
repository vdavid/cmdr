...
carries this exact shape; release CI (`release.yml`'s `bump-tap` job) rewrites only its `version` + `sha256` on each
release. So edit shape changes here; don't hand-bump the version (CI does it). The tap stays canonical until
`vdavid/cmdr` clears Homebrew's notability bar, at which point a resubmission to `Homebrew/homebrew-cask` makes that the
