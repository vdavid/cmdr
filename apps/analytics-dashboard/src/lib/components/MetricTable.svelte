<!-- A simple two-column "label / value" table for ranked breakdowns (referrers, pages, channels, etc.). -->
<script lang="ts">
    ...
    const {
        ...
        colLabel,
        colValue,
    }: { items: Array<{ x: string; y: number }>; colLabel: string; colValue: string } = $props()
</script>
