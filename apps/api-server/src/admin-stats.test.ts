...
describe('GET /admin/stats', () => {
  ...
  it('returns 401 with wrong token', async () => {
    const res = await app.request('/admin/stats', { headers: { Authorization: 'Bearer wrong-token' } }, baseBindings)

    expect(res.status).toBe(401)
  })

  it('returns 401 with malformed auth header', async () => {
    const res = await app.request('/admin/stats', { headers: { Authorization: 'Basic dGVzdDp0ZXN0' } }, baseBindings)
