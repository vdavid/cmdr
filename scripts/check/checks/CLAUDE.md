...
- Warn-only scanners with JSON allowlists: `file-length.go`, `claude-md-length.go`, `invariant-density.go`,
  `e2e-durations.go`, the two bundle-size lanes over shared `bundle-size-baseline.go`, `lock-poison.go`'s second lane,
  and the two copy-paste lanes over shared `jscpd.go`. Error-level doc-graph checks: `docs-reachable.go` (+
