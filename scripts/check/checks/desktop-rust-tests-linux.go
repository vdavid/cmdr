...
func RunRustTestsLinux(ctx *CheckContext) (CheckResult, error) {
	...
	cmd := exec.Command("docker", "run", "--rm",
		...
		"rust:latest",
		"sh", "-c", "apt-get update && apt-get install -y libgtk-3-dev libwebkit2gtk-4.1-dev libayatana-appindicator3-dev librsvg2-dev libacl1-dev && cargo test --no-fail-fast 2>&1")
