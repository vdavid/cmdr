...
var cargoAuditIgnoredAdvisories = []string{
	...
	"RUSTSEC-2026-0192", // ttf-parser unmaintained (abandoned upstream, no patched release, successor is `skrifa`): reached only via `pdf-extract` → `lopdf`, whose latest still requires ^0.25.1. The advisory's underlying complexity DoS needs a hostile FONT passed to `lopdf::FontData::new` (a PDF-authoring API), which nothing here calls — `inspect_file`'s PDF path never constructs one. Remove when `lopdf` moves off it.
