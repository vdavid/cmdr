...
- **Run from repo root via `pnpm check`.** Positional args select checks/apps/groups in any mix; named checks run even
  if slow/CI-only, app/group selectors keep the default lanes. `ValidateCheckNames` fails startup on a name shadowing a
  reserved group/app keyword.
...
- **A check names its Docker fixtures in `NeedsContainers []StackMode`**, resolved against `stacklease`'s registry
  (`smb`, `sftp`). Each stack gets its own machine-wide lease, so concurrent worktree runs coexist and a stack downs
  only at its last holder. ❌ Don't reintroduce per-check teardown, and ❌ don't move SMB's `/tmp/cmdr-smb.lock` or
...
- **The lane's filter comes from one fixture table** (`checks/fixture-lane-coverage.go`), which
  ...
  PARSE the filterset and the lane dies. An unmatched `test(prefix)` is fine.
