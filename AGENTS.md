...
- ❌ **No TS callback type with two or more confusable positional parameters** (same type, or an unresolved generic):
  ...
  wrong one, and same-typed positional params are freely swappable at the call site. Fix: a single named object payload
  (`(args: { volumeId: string; targetPath: string }) => void`), so a dropped or reordered field fails to compile.
  Enforced by `cmdr/no-confusable-callback-params`; opt out per-line with a reasoned `eslint-disable-next-line` when the
  shape genuinely mirrors a fixed, unconverted external or production signature (a platform API, or a positional pair
  with exactly one real, untouched call site).
